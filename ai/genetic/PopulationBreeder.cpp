...
void PopulationBreeder::breedPopulation(Crossbreedable **inPopulation, int inPopulationSize, float inMutationProb,
                                        float inMaxMutationMagnitude, PopulationDeleter *inDeleter)
{
    ...
    int remainder = inPopulationSize - (numToBreed * (numToBreed + 1)) / 2;
    ...
    for (int a = 0; a < numToBreed; a++)
    {
        for (int b = a + 1; b < numToBreed; b++)
        {
            ...
            void *offspring =
                inPopulation[a]->crossbreed(inPopulation[b], 0.5f, inMutationProb, inMaxMutationMagnitude);
