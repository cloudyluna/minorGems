...
void PopulationSorter::sortPopulation(PopulationMember **inPopulation, double *inScores, int inPopulationSize,
                                      char inDecreasingOrder)
{
    ...
    for (int t = 0; t < inPopulationSize; t++)
    {
        ...
        {
            if (!inDecreasingOrder && inScores[u] < best || inDecreasingOrder && inScores[u] > best)
            {
                best = inScores[u];
                bestInd = u;
            }
