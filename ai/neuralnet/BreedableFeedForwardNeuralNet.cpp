 ...
 */
inline double mutateValue(double inValue, double inMaxMutationMagnitude, double inRangeMax, double inRangeMin,
                          RandomSource *inRandSource)
{

    // pick a mutated value in the range
    double mutatedValue = inRandSource->getRandomFloat() * (inRangeMax - inRangeMin) + inRangeMin;

    // use inMaxMutationMagnitude to take a weighted average
    // from current value and mutatedValue
    return inMaxMutationMagnitude * mutatedValue + (1 - inMaxMutationMagnitude) * inValue;
}
 ...
 */
inline int pickUnmarkedIndex(char *inMarkers, int inLength, RandomSource *inRandSource)
{
 ...
 *   or -1 if no unmarked spots exist.
 */
inline int getIndexMaxMagnitude(char *inMarkers, double *inValues, int inLength)
{
...
BreedableFeedForwardNeuralNet::BreedableFeedForwardNeuralNet(int inNumInputs, int inNumHiddenLayers,
                                                             int *inNeuronsPerLayer, int inNumOutputs,
                                                             RandomSource *inRandSource)
    : FeedForwardNeuralNet(inNumInputs, inNumHiddenLayers, inNeuronsPerLayer, inNumOutputs), mRandSource(inRandSource),
      mMaxWeight(1.0), mMinWeight(-1.0), mCrossbreedingMethod(1)
{
...
BreedableFeedForwardNeuralNet::BreedableFeedForwardNeuralNet(FILE *inInputFile, RandomSource *inRandSource)
    : FeedForwardNeuralNet(inInputFile), Crossbreedable(inInputFile), mRandSource(inRandSource), mCrossbreedingMethod(1)
{
...
void BreedableFeedForwardNeuralNet::setCrossbreedingMethod(const char *inCrossbreedingMethod)
{
...
void BreedableFeedForwardNeuralNet::setCrossbreedingMethod(int inCrossbreedingMethod)
{

    mCrossbreedingMethod = inCrossbreedingMethod;
}

void *BreedableFeedForwardNeuralNet::crossbreed(Crossbreedable *inOther, float inFractionOther, float inMutationProb,
                                                float inMaxMutationMagnitude)
{

    BreedableFeedForwardNeuralNet *otherNet = dynamic_cast<BreedableFeedForwardNeuralNet *>(inOther);
    ...
    BreedableFeedForwardNeuralNet *offspring = new BreedableFeedForwardNeuralNet(
        mNumInputs, mNumHiddenLayers, &(mNeuronsPerLayer[1]), mNumOutputs, mRandSource);
    ...
    if (mCrossbreedingMethod == 1)
    { // forward paths method
        ...
        while (input != -1)
        {
            ...
            for (int j = 1; j < totalNumLayers; j++)
            {
                ...
                {
                    double parentWeight = parent->getWeight(currentLayer, currentLayerNeuron, k);
                    offspring->setWeight(currentLayer, currentLayerNeuron, k, parentWeight);
                ...
                int nextLayerNeuron = getIndexMaxMagnitude(markers[j], nextLayerWeights, mNeuronsPerLayer[j]);
                ...
                if (nextLayerNeuron != -1)
                {
                    ...
                    offspring->setThreshold(j, nextLayerNeuron, parent->getThreshold(j, nextLayerNeuron));
                    offspring->setThresholdValue(j, nextLayerNeuron, parent->getThresholdValue(j, nextLayerNeuron));
        ...
        for (i = 1; i < totalNumLayers; i++)
        {
            ...
            int unmarked = pickUnmarkedIndex(markers[i], mNeuronsPerLayer[i], mRandSource);
            while (unmarked != -1)
            {
                ...
                offspring->setThreshold(i, unmarked, parent->getThreshold(i, unmarked));
                offspring->setThresholdValue(i, unmarked, parent->getThresholdValue(i, unmarked));
                ...
                if (i != totalNumLayers - 1)
                {
                    ...
                    {
                        offspring->setWeight(i, unmarked, k, parent->getWeight(i, unmarked, k));
                    }
                }

                // look for another unmarked neuron in this layer
                unmarked = pickUnmarkedIndex(markers[i], mNeuronsPerLayer[i], mRandSource);
            }
        }
    ...
    */
    ...
    else if (mCrossbreedingMethod == 0)
    {
        ...
        for (int i = 0; i < totalNumLayers; i++)
        {
            ...
            for (int j = 0; j < mNeuronsPerLayer[i]; j++)
            {
                ...
                // copy neuron into offspring
                offspring->setThreshold(i, j, parent->getThreshold(i, j));
                offspring->setThresholdValue(i, j, parent->getThresholdValue(i, j));
                ...
                if (i != totalNumLayers - 1)
                {
                    ...
                    for (int k = 0; k < mNeuronsPerLayer[i + 1]; k++)
                    {
                        offspring->setWeight(i, j, k, parent->getWeight(i, j, k));
                    }
                }
            }
        }
    ...
    else if (mCrossbreedingMethod == 4)
    {
        ...
        float parentARangeStart = mRandSource->getRandomFloat() * inFractionOther;
        ...
        for (int i = 0; i < totalNumLayers; i++)
        {
            ...
            for (int j = 0; j < mNeuronsPerLayer[i]; j++)
            {
                ...
                if (j < parentARangeStart * mNeuronsPerLayer[i] || j > parentARangeEnd * mNeuronsPerLayer[i])
                {
                    parent = otherNet;
                }
                else
    ...
    else if (mCrossbreedingMethod == 6)
    {
        ...
        for (int i = 0; i < totalNumLayers; i++)
        {
            ...
            for (int j = 0; j < mNeuronsPerLayer[i]; j++)
            {
                ...
                // take average
                double averageThresholdValue = (1 - inFractionOther) * this->getThresholdValue(i, j) +
                ...
                if (i != totalNumLayers - 1)
                {
                    ...
                    {
                        // take average
                        double averageWeight = (1 - inFractionOther) * this->getWeight(i, j, k) +
                                               inFractionOther * otherNet->getWeight(i, j, k);
    ...
    else if (mCrossbreedingMethod == 7)
    {
        ...
        float parent1RangeStart = mRandSource->getRandomFloat() * inFractionOther;
        ...
        memcpy((void *)offspringString, (void *)parent2String, lengthInBytes);

        // then copy parent 1 range into offspring
        memcpy((char *)offspringString + parent1IndStart, (char *)parent1String + parent1IndStart, parent1RangeLength);
        ...
        for (i = 0; i < lengthInBytes; i++)
        {
            for (int b = 0; b < 8; b++)
            {
                if (mRandSource->getRandomFloat() <= inMutationProb)
                {
                    ...
                    offspringByteString[i] = offspringByteString[i] ^ (1 << b);
                }
            }
        }
...
void BreedableFeedForwardNeuralNet::mutate(float inMutationProb, float inMaxMutationMagnitude)
{
            ...
            for (int j = 0; j < mNeuronsPerLayer[i]; j++)
            {
                ...
                if (mRandSource->getRandomFloat() <= inMutationProb)
                {
                    ...
                    mNeuronThresholdValues[i][j] =
                        mutateValue(mNeuronThresholdValues[i][j], inMaxMutationMagnitude, mMaxLayerThreshold[i],
                                    mMinLayerThreshold[i], mRandSource);
                }
                ...
                if (i < totalNumLayers - 1)
                {
                    for (int k = 0; k < numInNextLayer; k++)
                    {
                        ...
                        if (mRandSource->getRandomFloat() <= inMutationProb)
                        {
                            weightsThisNeuron[k] = mutateValue(weightsThisNeuron[k], inMaxMutationMagnitude, mMaxWeight,
                                                               mMinWeight, mRandSource);
                        }
                    }
                }
            }
