...
FeedForwardNeuralNet::FeedForwardNeuralNet(int inNumInputs, int inNumHiddenLayers, int *inNeuronsPerLayer,
                                           int inNumOutputs)
    ...
    for (i = 0; i < totalNumLayers; i++)
    {
        ...
        for (int j = 0; j < mNeuronsPerLayer[i]; j++)
        {
            ...
            if (i < totalNumLayers - 1)
            {
                ...
                memcpy(mWeights[i][j], tempWeights, neuronsNextLayer * sizeof(double));
            }
        }
        delete[] tempWeights;
    }
...
void FeedForwardNeuralNet::run(double *inInputs, double *outOutputs)
{
    ...
    register int i;
    memcpy((void *)currentValues, (void *)inInputs, mNeuronsPerLayer[0] * sizeof(double));
    ...
    for (i = 1; i < mNumHiddenLayers + 2; i++)
    {
        ...
        for (int k = 0; k < mNeuronsPerLayer[i - 1]; k++)
        {
            ...
            */
            ...
            for (j = 5; j < numNeuronsThisLayer; j += 6)
            {
                ...
                newValues[backj] += weightsLeavingPrevious[backj] * currentValuePrevious;
                backj++;
                newValues[backj] += weightsLeavingPrevious[backj] * currentValuePrevious;
                backj++;
                newValues[backj] += weightsLeavingPrevious[backj] * currentValuePrevious;
                ...
                backj++;
                newValues[backj] += weightsLeavingPrevious[backj] * currentValuePrevious;
                newValues[j] += weightsLeavingPrevious[j] * currentValuePrevious;
            }
            // finish remainder
        ...
        for (j = 0; j < mNeuronsPerLayer[i]; j++)
        {
            newValues[j] = mNeuronThresholds[i][j]->apply(newValues[j], mNeuronThresholdValues[i][j]);
        }
    ...
    memcpy((void *)outOutputs, (void *)currentValues, mNumOutputs * sizeof(double));
    delete[] currentValues;
}
