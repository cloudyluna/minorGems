...
// prototypes:
void loadFromFile(double **inInputs, double *inOutputs, int inNumExamples, int inNumInputs, FILE *inFile);
...
int main(char inNumArgs, char **inArgs)
{
    ...
    loadFromFile(inputs, outputs, numExamples, numInputs, exampleFile);

    for (i = 0; i < numExamples; i++)
    {
        examples[i] = new DoubleTrainingExample(inputs[i], &(outputs[i]), numInputs, 1);
    }

    int numMutationProbSteps = (int)((probRangeEnd - probRangeStart) / mutationProbStepSize);
    int numMutationMagnitudeSteps = (int)((magRangeEnd - magRangeStart) / mutationMagnitudeStepSize);
    ...
    {
        float currentMutationMagnitude = magRangeStart + m * mutationMagnitudeStepSize;
        for (int p = 1; p <= numMutationProbSteps; p++)
        {
            float currentMutationProb = probRangeStart + p * mutationProbStepSize;
            ...
            for (int s = 0; s < averagingSetSize; s++)
            {
                ...
                BreedableFeedForwardNeuralNet **population = new BreedableFeedForwardNeuralNet *[populationSize];

                for (i = 0; i < populationSize; i++)
                {
                    population[i] =
                        new BreedableFeedForwardNeuralNet(numInputs, numHiddenLayers, neuronsPerLayer, 1, randSource);
                ...
                BreedingDoubleExampleTrainer *trainer = new BreedingDoubleExampleTrainer();
                ...
                int result = 1;
                result = trainer->train(numRounds, currentMutationProb, currentMutationMagnitude, roundErrors);
            ...
            averageConvergenceRate[m - 1][p - 1] = averageConvergenceRate[m - 1][p - 1] / averagingSetSize;
    ...
    fprintf(statsFile, "Min convergence rate of %f for magnitude %f and prob %f\n", minConvergenceRate,
            magRangeStart + mMin * mutationMagnitudeStepSize, probRangeStart + pMin * mutationProbStepSize);
