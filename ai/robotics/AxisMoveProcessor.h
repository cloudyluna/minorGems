...
class AxisMoveProcessor : public MoveProcessor
{
  ...
  private:
     ...
     */
    void sendMessage(unsigned char inCommandTypeByte, unsigned char inParameterByte);
...
inline AxisMoveProcessor::AxisMoveProcessor(HostAddress *inCameraAddress) : mCameraAddress(inCameraAddress)
{
}
...
inline void AxisMoveProcessor::waitForReady()
{
    ...
    char ready = false;
    char *pinReadString = "/axis-cgi/io/input.cgi?monitor=1";

    Socket *mCameraSocket = SocketClient::connectToServer(mCameraAddress);
    ...
    mSocketStream->write((unsigned char *)requestBuffer, strlen(requestBuffer));
...
inline void AxisMoveProcessor::sendMessage(unsigned char inCommandTypeByte, unsigned char inParameterByte)
{
...
inline void AxisMoveProcessor::translateForward(double inDistanceInCentimeters)
{
...
inline void AxisMoveProcessor::rotateClockwise(double inRotationFraction)
{
