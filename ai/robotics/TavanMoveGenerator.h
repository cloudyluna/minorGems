...
inline TavanMoveGenerator::TavanMoveGenerator()
    : mLeftRect(new ImageRectangle(0, 0.333, 0, 1)), mRightRect(new ImageRectangle(0.667, 1, 0, 1)),
...
inline void TavanMoveGenerator::generateMoves(Image *inLeftImage, Image *inRightImage, Image *inStereoData,
                                              MoveProcessor *inMoveProcessor)
{
    if (!mHaveTwoImages)
    {
    ...
    else
    {
        ...
        Image *threeChannelImage = ImageColorConverter::grayscaleToRGB(diffMap);
        ...
        double totalDiff = fabs(leftCenterDiff) + fabs(centerRightDiff) + fabs(rightLeftDiff);
