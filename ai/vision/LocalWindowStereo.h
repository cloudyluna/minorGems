...
class LocalWindowStereo : public PartialStereo
{
  ...
  public:
     ...
     *   the right image.
     */
    LocalWindowStereo(int inMaxDisparity, int inWindowSize, RandomSource *inRandSource);
...
inline LocalWindowStereo::LocalWindowStereo(int inMaxDisparity, int inWindowSize, RandomSource *inRandSource)
    : PartialStereo(inMaxDisparity), mWindowSize(inWindowSize), mRandSource(inRandSource)
{
}
...
inline Stereo *LocalWindowStereo::copy()
{
    ...
    LocalWindowStereo *returnValue = new LocalWindowStereo(mMaxDisparity, mWindowSize, mRandSource);
...
inline Image *LocalWindowStereo::computeDepthMap(Image *inLeft, Image *inRight)
{
    ...
    unsigned char *leftChannel = ImageColorConverter::grayscaleToByteArray(inLeft, mChannelNumber);
    unsigned char *rightChannel = ImageColorConverter::grayscaleToByteArray(inRight, mChannelNumber);
    ...
    { // for each pixel in image
        ...
        for (int x = xStart; x <= xEnd; x++)
        {
            ...
            for (int d = 0; d <= mMaxDisparity; d++)
            {
                ...
                for (int delY = -startBox; delY <= boxRad; delY++)
                {
                    ...
                    for (int delX = -startBox; delX <= boxRad; delX++)
                    {
                        ...
                        if (xPlusDelX - d > 0)
                        { // pixel lands in right image when moved by d
                            valRight = rightChannel[windowPixInd - d];
                        }
                        else
                        { // pixel lands outside R-image when displaced by d
                            ...
                            valRight = mRandSource->getRandomBoundedInt(0, 255);
                        }
                ...
                if (thisCostL2 < costOfBestL2)
                { // check if new better L2 displ. found
                    bestDispL2 = d;
            ...
            outChannel[yOffset[y] + x] = (double)bestDispL2 / (double)mMaxDisparity;
