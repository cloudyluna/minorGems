...
inline NetworkedStereo::NetworkedStereo(SocketStream **inStreams, int inNumParts)
    : Stereo(1), mStreams(inStreams), mNumParts(inNumParts), mStereoComputers(new NetworkedPartialStereo *[mNumParts])
{
...
inline Image *NetworkedStereo::computeDepthMap(Image *inLeft, Image *inRight)
{
    ...
    for (i = 0; i < mNumParts; i++)
    {
        ...
        threads[i] = new StereoThread(mStereoComputers[i], inLeft, inRight, &(outImages[i]));
