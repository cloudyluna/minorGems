...
P_M::P_M(char *fileName, int *status)
{
    ...
    { // each byte contains 8 pixes or-ed together
...
P_M::P_M(P_M_format f, int w, int h, unsigned char *b, int maxGry)
    : format(f), width(w), height(h), buffer(new unsigned char[w * h]), rowOffsets(new int[h]), maxGray(maxGry)
{
...
char passComment(FILE *inFile)
{ // pass up a comment if next in input
