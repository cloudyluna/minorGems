...
class StereoClient : public Thread
{
  ...
  public:
     ...
     */
    StereoClient(ImageSource *inLeftImageSource, ImageSource *inRightImageSource, Stereo *inStereo,
                 StereoProcessor *inStereoProcessor, ImagePairSideProcessor *inImageProcessor);
...
inline StereoClient::StereoClient(ImageSource *inLeftImageSource, ImageSource *inRightImageSource, Stereo *inStereo,
                                  StereoProcessor *inStereoProcessor, ImagePairSideProcessor *inImageProcessor)

    : mLeftImageSource(inLeftImageSource), mRightImageSource(inRightImageSource), mStereo(inStereo),
      mImageProcessor(inImageProcessor), mStereoProcessor(inStereoProcessor), mLeftImageBuffer(new CircularBuffer(1)),
      mRightImageBuffer(new CircularBuffer(1)), mStopFlag(false), mStopFlagLock(new MutexLock())
{
...
inline void StereoClient::run()
{
    ...
    while (running)
    {
        ...
        Image *leftGrayImage = ImageColorConverter::RGBToGrayscale(leftImage);
        Image *rightGrayImage = ImageColorConverter::RGBToGrayscale(rightImage);

        mImageProcessor->process(leftImage, rightImage);

        Image *depthMap = mStereo->computeDepthMap(leftGrayImage, rightGrayImage);
