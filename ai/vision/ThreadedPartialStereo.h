...
inline ThreadedPartialStereo::ThreadedPartialStereo(PartialStereo *inStereo, int inNumParts)
    : Stereo(inStereo->getMaxDisparity()), mStereo(inStereo), mNumParts(inNumParts)
{
}
...
inline Stereo *ThreadedPartialStereo::copy()
{
    ...
    ThreadedPartialStereo *returnValue = new ThreadedPartialStereo((PartialStereo *)(mStereo->copy()), mNumParts);

    return returnValue;
}

inline Image *ThreadedPartialStereo::computeDepthMap(Image *inLeft, Image *inRight)
{
    ...
    for (i = 0; i < mNumParts; i++)
    {
        ...
        threads[i] = new StereoThread(stereoCopies[i], inLeft, inRight, &(outImages[i]));
