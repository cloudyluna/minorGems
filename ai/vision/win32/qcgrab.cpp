...
// handled here.
class CDriver : public IDispatchImpl<_IVideoPortalEvents, &IID__IVideoPortalEvents, &LIBID_VPORTAL2Lib>,
                public CComObjectRoot
{
  public:
    ...
    STDMETHOD(PortalNotification)(long lMsg, long lParam1, long lParam2, long lParam3)
    {
...
BOOL InitializeVideo(HWND hParent)
{
    ...
    HWND hWndCtl = ::CreateWindow("AtlAxWin", OLE2T(strGUIDVideoPortalActiveXControl), WS_CHILD | WS_VISIBLE | WS_GROUP,
                                  0, 0, 400, 400, hParent, NULL, ::GetModuleHandle(NULL), NULL);
    ...
    if (FAILED(AtlAdvise(gpVideo, gpDriver->GetUnknown(), IID__IVideoPortalEvents, &gdwAdviseCookie)))
    {
        gpVideo->Release();
        gpVideo = NULL;
        return FALSE;
    ...
    if (FAILED(gpVideo->EnableUIElements(UIELEMENT_STATUSBAR, 0, TRUE, &lResult)))
    {
    }
