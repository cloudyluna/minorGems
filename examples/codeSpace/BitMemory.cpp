...
unsigned char *BitMemory::getMemoryRange(unsigned long inRangeStart, unsigned long inRangeEnd,
                                         unsigned long *outNumCharsReturned)
{
...
void BitMemory::setMemoryRange(unsigned long inRangeStart, unsigned long inRangeEnd, unsigned char *inRangeContents)
{
    ...
    for (int m = inRangeStart; m <= inRangeEnd; m++)
    {
        ...
        inputBit = (char)((inRangeContents[block] >> shiftAmount) & 0x01);
...
void BitMemory::clearMemoryRange(unsigned long inRangeStart, unsigned long inRangeEnd)
{

    // check memory range
    if (inRangeStart < 0 || inRangeEnd > mNumBits || inRangeStart > inRangeEnd)
    {
