...
void CommonInstructionSet::copyi(BitMemory *inMemory, unsigned long inSrc, unsigned long inDest, unsigned long inLength)
{
    ...
    unsigned char *srcValues = inMemory->getMemoryRange(inSrc, inSrc + inLength - 1, &numChars);

    inMemory->setMemoryRange(inDest, inDest + inLength - 1, srcValues);
    delete[] srcValues;
}

void CommonInstructionSet::andi(BitMemory *inMemory, unsigned long inSrc1, unsigned long inSrc2, unsigned long inDest,
                                unsigned long inLength)
{
    ...
    unsigned long numChars;
    unsigned char *src1Values = inMemory->getMemoryRange(inSrc1, inSrc1 + inLength - 1, &numChars);
    unsigned char *src2Values = inMemory->getMemoryRange(inSrc2, inSrc2 + inLength - 1, &numChars);
...
void CommonInstructionSet::noti(BitMemory *inMemory, unsigned long inSrc, unsigned long inDest, unsigned long inLength)
{
