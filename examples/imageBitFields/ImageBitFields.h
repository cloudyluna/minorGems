...
class ImageBitFields
{
  ...
  public:
     ...
     *   Must be destroyed by caller.
     */
    static BitField **imageToBitFields(Image *inImage, int inChannel, int inNumFields);
     ...
     *   Must be destroyed by caller.
     */
    static BitField ***imageToBitFields(Image *inImage, int inNumFieldsPerChannel);
     ...
     *   Must be destroyed by caller.
     */
    static Image *bitFieldsToImage(BitField **inFields, int inNumFields);
     ...
     *   Must be destroyed by caller.
     */
    static Image *bitFieldsToImage(BitField ***inFields, int inNumChannels, int inNumFieldsPerChannel);
};

inline BitField **ImageBitFields::imageToBitFields(Image *inImage, int inChannel, int inNumFields)
{

    if (inNumFields > 32 || inNumFields < 0)
    {
        printf("ImageBitFields::imageToBitFields, inNumFields ");
    ...
    for (p = 0; p < numPixels; p++)
    {
        ...
        // number of bits.
        unsigned int pixelValue = (unsigned int)(maxIntPixelValue * pixels[p]);
        ...
        for (i = 0; i < inNumFields; i++)
        {
            ...
            fields[i][p] = (char)(pixelValue >> (inNumFields - 1 - i) & 0x1);
        }
    }
...
inline BitField ***ImageBitFields::imageToBitFields(Image *inImage, int inNumFieldsPerChannel)
{

    if (inNumFieldsPerChannel > 32 || inNumFieldsPerChannel < 0)
    {
        printf("ImageBitFields::imageToBitFields, inNumFieldsPerChannel ");
        printf("must be in [0,32]\n");
    ...
    {
        returnFields[i] = imageToBitFields(inImage, i, inNumFieldsPerChannel);
    }

    return returnFields;
}

inline Image *ImageBitFields::bitFieldsToImage(BitField **inFields, int inNumFields)
{

    if (inNumFields > 32 || inNumFields < 0)
    {
        printf("ImageBitFields::bitFieldsToImage, inNumFields ");
        printf("must be in [0,32]\n");
    ...
    for (p = 0; p < numPixels; p++)
    {
        ...
        for (i = 0; i < inNumFields; i++)
        {
            ...
            pixelValue = pixelValue | (fields[i][p] << (inNumFields - 1 - i));
        }
...
inline Image *ImageBitFields::bitFieldsToImage(BitField ***inFields, int inNumChannels, int inNumFieldsPerChannel)
{

    if (inNumFieldsPerChannel > 32 || inNumFieldsPerChannel < 0)
    {
        printf("ImageBitFields::bitFieldsToImage, inNumFieldsPerChannel ");
        printf("must be in [0,32]\n");
    ...
    {
        images[i] = bitFieldsToImage(inFields[i], inNumFieldsPerChannel);
    }
