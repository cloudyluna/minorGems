...
int main(char inNumArgs, char **inArgs)
{
    ...
    BitField ***fields = ImageBitFields::imageToBitFields(inImage, 8);
        ...
        for (int b = 0; b < 8; b++)
        {
            ...
            for (int blockY = 0; blockY < h / blockSize; blockY++)
            {
                ...
                for (int blockX = 0; blockX < w / blockSize; blockX++)
                {
                    ...
                    float blockWeight = (float)valSum / (float)(blockSize * blockSize);
                    ...
                    float complexity = (float)numBlockChanges / (float)maxNumBlockChanges;
    ...
    printf("fraction of blocks compressable:  %f\n",
           (numBlocksCompressed + numBlocksHighOrLow) / (float)numBlocksTotal);
    ...
    */
    ...
    // convert back
    Image *outImage = ImageBitFields::bitFieldsToImage(fields, 3, 8);
