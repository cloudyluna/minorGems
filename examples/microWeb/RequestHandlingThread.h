...
class RequestHandlingThread : public Thread
{
  ...
  public:
     ...
     */
    RequestHandlingThread(Socket *inSocket, char *inMimeString, char *inRootPath);
  ...
  private:
     ...
     *   or NULL.
     */
    void sendNotFoundPage(SocketStream *inStream, char *inFileName);
...
inline RequestHandlingThread::RequestHandlingThread(Socket *inSocket, char *inMimeString, char *inRootPath)
    : mSocket(inSocket), mMimeString(inMimeString), mRootPathString(inRootPath), mDoneLock(new MutexLock()),
      mDone(false)
{
}
...
inline void RequestHandlingThread::run()
{
    ...
    else
    {
        ...
        numRead = sscanf(&(requestBuffer[3]), "%s", filePathBuffer);
    ...
    if (!error)
    {
        ...
        if (file->exists())
        {
            ...
            sockStream->write((unsigned char *)responseBuffer, strlen(responseBuffer));
...
inline void RequestHandlingThread::sendNotFoundPage(SocketStream *inStream, char *inFileName)
{
    ...
    /*
      ...
      Server: GWS/1.10
/	  Connection: close
      Set-Cookie: PREF=ID=3300d4623bf73a57:TM=989605576:LM=989605576; domain=.google.com; path=/; expires=Sun,
17-Jan-2038 19:14:07 GMT Content-Length: 142 Content-Type: text/html
    ...
    */
    ...
    {
        sprintf(buffer,
                "HTTP/1.0 404 Not Found\r\n\r\n<HTML><BODY><H1>404 Not Found</H1>The requested file <b>%s</b> was not "
                "found</BODY></HTML>\r\n",
                inFileName);
    }
    else
    {
        sprintf(buffer, "HTTP/1.0 404 Not Found\r\n\r\n<HTML><BODY><H1>404 Not Found</H1>The requested file was not "
                        "found</BODY></HTML>\r\n");
    }
...
inline void RequestHandlingThread::sendBadRequest(SocketStream *inStream)
{
    ...
    */
    ...
    sprintf(
        buffer,
        "<HTML><BODY><H1>400 Bad Request</H1>Your client has issued a malformed or illegal request.</BODY></HTML>\r\n");
