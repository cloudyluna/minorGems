...
class ReceiveThread : public Thread
{
  ...
  public:
    ReceiveThread(Socket *inListenSocket, Socket *inForwardSocket, int inID);
...
ReceiveThread::ReceiveThread(Socket *inListenSocket, Socket *inForwardSocket, int inID)
    : mListenSocket(inListenSocket), mForwardSocket(inForwardSocket), mID(inID)
{
}
...
class SendThread : public Thread
{
  ...
  public:
    SendThread(Socket *inListenSocket, char *inForwardAddress, int inForwardPort, int inID);
...
SendThread::SendThread(Socket *inListenSocket, char *inForwardAddress, int inForwardPort, int inID)
    : mListenSocket(inListenSocket), mForwardAddress(inForwardAddress), mForwardPort(inForwardPort), mID(inID)
{
}
...
void SendThread::run()
{
    ...
    printf("Establishing a connection to %s:%d\n", mForwardAddress, mForwardPort);

    HostAddress *address = new HostAddress(stringDuplicate(mForwardAddress), mForwardPort);
    ...
    if (forwardSocket != NULL)
    {
        ...
        ReceiveThread *thread = new ReceiveThread(mListenSocket, forwardSocket, mID);
...
ThreadManager::ThreadManager() : mLock(new MutexLock()), mThreads(new SimpleVector<Thread *>())
{
}
...
int main(char inNumArgs, char **inArgs)
{
    ...
    while (true)
    {
        ...
        if (listenSocket != NULL)
        {
            ...
            SendThread *thread = new SendThread(listenSocket, forwardAddress, forwardPort, nextID);
            nextID++;
