...
class EpicycleTerrainPath : public TerrainPath
{
  public:
     ...
     */
    EpicycleTerrainPath(double inRadius, double inRadiusRatio, double inRotationRatio, double inRotationOffset);
    ...
    virtual void getPosition(double inTime, double *outX, double *outY);
...
inline EpicycleTerrainPath::EpicycleTerrainPath(double inRadius, double inRadiusRatio, double inRotationRatio,
                                                double inRotationOffset)
    : mRadius(inRadius), mRadiusRatio(inRadiusRatio), mRotationRatio(inRotationRatio), mRotationOffset(0.5)
{
...
inline void EpicycleTerrainPath::setRotationRatio(double inRotationRatio)
{
...
inline void EpicycleTerrainPath::getPosition(double inTime, double *outX, double *outY)
{
    ...
    double majorScaledTime = 2 * (inTime + mRotationOffset) * M_PI;
    double minorScaledTime = 2 * (inTime)*M_PI * mRotationRatio;

    // first, rotate around the minor orbit
    double minorX = (cos(minorScaledTime) * minorRadius);
    double minorY = (sin(minorScaledTime) * minorRadius);

    // compute the major contribution
    double majorX = (cos(majorScaledTime) * majorRadius);
    double majorY = (sin(majorScaledTime) * majorRadius);
...
inline void EpicycleTerrainPath::checkValues()
{
    ...
    {
        printf("Radius of a EpicycleTerrainPath must be in [0, 0.5]\n");
    ...
    if (mRadiusRatio < 0)
    {
        printf("Radius ratio of a EpicycleTerrainPath must be");
        printf(" in [0, DBL_MAX]\n");
    ...
    if (mRotationRatio < 0)
    {
        printf("Rotation ratio of a EpicycleTerrainPath must be");
        printf(" in [0, DBL_MAX]\n");
