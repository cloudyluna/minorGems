...
inline WaveTerrain::WaveTerrain(double **inHeightMap, int inWidth, int inHeight)
...
inline double WaveTerrain::getSample(double inX, double inY)
{
    ...
    double lowYAverage = (highXWeight * mHeightMap[lowY][highX]) + ((1 - highXWeight) * mHeightMap[lowY][lowX]);

    // average of two points with highY coordinate
    double highYAverage = (highXWeight * mHeightMap[highY][highX]) + ((1 - highXWeight) * mHeightMap[highY][lowX]);

    // average in y direction
    double returnValue = (highYWeight * highYAverage) + ((1 - highYWeight) * lowYAverage);
