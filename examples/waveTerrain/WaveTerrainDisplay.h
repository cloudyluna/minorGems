...
class WaveTerrainDisplay : public Thread
{
  public:
     ...
     */
    WaveTerrainDisplay(WaveTerrain *inTerrain, TerrainPath *inPath, int inWidth, int inHeight);
...
inline WaveTerrainDisplay::WaveTerrainDisplay(WaveTerrain *inTerrain, TerrainPath *inPath, int inWidth, int inHeight)
    : mTerrain(inTerrain), mPath(inPath), mWidth(inWidth), mHeight(inHeight),
      mScreen(new ScreenGraphics(inWidth, inHeight)), mStopFlag(false), mNumPathSamples(250)
{
    ...
    for (int y = 0; y < mHeight; y++)
    {
        ...
        {
            double sample = mTerrain->getSample(x * invWidth, y * invHeight);

            int green = (int)(255 * sample);
...
inline void WaveTerrainDisplay::run()
{
    ...
    while (!mStopFlag)
    {
        ...
        memcpy(mPixels, mTerrainPixels, mWidth * mHeight * sizeof(long));
