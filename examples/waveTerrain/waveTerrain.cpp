...
int main()
{
    ...
    */
    for (y = 0; y < high; y++)
    {
        ...
        for (x = 0; x < wide; x++)
        {
            ...
            */
            ...
            map[y][x] =
                sin(30 * sqrt(x / ((double)wide) * x / ((double)wide) + y / ((double)high) * y / ((double)high)));
            map[y][x] *= sin(30 * sqrt((x - wide) / ((double)wide) * (x - wide) / ((double)wide) +
    ...
    EpicycleTerrainPath *path = new EpicycleTerrainPath(0.5, 0, 0, 0);

    WaveTerrainDisplay *display = new WaveTerrainDisplay(terrain, path, 100, 100);
    ...
    while (true)
    {
        ...
        for (int k = 0; k < soundBufferSize; k++)
        {
            ...
            path->setRotationRatio(0.1 * sin(i * 0.00001 + (3 / 2) * M_PI) + 4);

            // path->setRadiusRatio( 1.0 );
            path->setRadiusRatio(10 * sin(i * 0.00002345 + (3 / 2) * M_PI) + 11);
            ...
            soundBuffer[k * 2] = (unsigned char)(shortSample >> 8 & 0x00FF);
            soundBuffer[k * 2 + 1] = (unsigned char)(shortSample & 0x00FF);
        }
