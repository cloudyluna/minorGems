...
static const char *binaryToAscii = "ABCDEFGHIJKLMNOPQRSTUVWXYZ"
...
char *base64Encode(unsigned char *inData, int inDataLength, char inBreakLines)
{
    ...
    for (int i = 0; i < inDataLength; i = i + 3)
    {
        ...
        if (i + 2 < inDataLength)
        {
            ...
            unsigned int block = inData[i] << 16 | inData[i + 1] << 8 | inData[i + 2];
        ...
        else
        {
            ...
            case 1: {
                // two digits, two pads
                unsigned int block = inData[i] << 16 | 0;
                unsigned int digitA = 0x3F & (block >> 18);
                unsigned int digitB = 0x3F & (block >> 12);
            ...
            case 2: {
                ...
                unsigned int block = inData[i] << 16 | inData[i + 1] << 8 | 0;
...
unsigned char *base64Decode(char *inBase64String, int *outDataLength)
{
    ...
    SimpleVector<unsigned char> *decodedVector = new SimpleVector<unsigned char>();

    int encodingLength = strlen(inBase64String);

    SimpleVector<unsigned char> *binaryEncodingVector = new SimpleVector<unsigned char>();
    ...
    for (i = 0; i < binaryEncodingLength; i = i + 4)
    {
        ...
        if (i + 3 < binaryEncodingLength)
        {
            ...
            unsigned int block = binaryEncoding[i] << 18 | binaryEncoding[i + 1] << 12 | binaryEncoding[i + 2] << 6 |
                                 binaryEncoding[i + 3];
        ...
        else
        {
            ...
            case 2: {
                ...
                unsigned int block = binaryEncoding[i] << 18 | binaryEncoding[i + 1] << 12 | 0;
            ...
            case 3: {
                ...
                unsigned int block =
                    binaryEncoding[i] << 18 | binaryEncoding[i + 1] << 12 | binaryEncoding[i + 2] << 6 | 0;
...
unsigned char *zipCompress(unsigned char *inData, int inDataLength, int *outCompressedDataLength)
{
    ...
    int cmp_status = compress(compressBuffer, &compLength, inData, inDataLength);
    if (cmp_status != Z_OK)
    {
                             ...
                             int inExpectedResultDataLength)
{
    ...
    unsigned char *dataBuffer = new unsigned char[inExpectedResultDataLength];

    mz_ulong actualDataLength = inExpectedResultDataLength;

    int cmp_status = uncompress(dataBuffer, &actualDataLength, inCompressedData, inCompressedDataLength);
