...
Font::Font(const char *inFileName, int inCharSpacing, int inSpaceWidth, char inFixedWidth, double inScaleFactor,
           int inFixedCharWidth)
    : mScaleFactor(inScaleFactor), mCharSpacing(inCharSpacing), mSpaceWidth(inSpaceWidth), mFixedWidth(inFixedWidth),
      mEnableKerning(true), mMinimumPositionPrecision(0)
{
    ...
    if (spriteImage != NULL)
    {
        ...
        unsigned char *spriteBytes = RGBAImage::getRGBABytes(spriteImage);
        ...
        for (int i = 0; i < 256; i++)
        {
            ...
            for (int y = 0; y < mSpriteHeight; y++)
            {
                for (int x = 0; x < mSpriteWidth; x++)
                {
                    ...
                    int imageIndex = (y + yOffset) * width + x + xOffset;
            ...
            if (!allTransparent)
            {
                ...
                Image *charImage = new Image(mSpriteWidth, mSpriteHeight, 4, false);
                ...
                mSpriteMap[i] = fillSprite(charImage);
                delete charImage;
            }
            else
            {
            ...
            else
            {
                ...
                for (int y = 0; y < mSpriteHeight; y++)
                {
                    for (int x = 0; x < mSpriteWidth; x++)
                    {
                        ...
                        unsigned char a = charRGBA[y * mSpriteWidth + x].comp.a;
                ...
                if (savedCharacterRGBA[i] != NULL)
                {
                    for (int y = 0; y < mSpriteHeight; y++)
                    {
                        ...
                        for (int x = 0; x < mSpriteWidth; x++)
                        {
                            ...
                            if (x < leftExtreme && savedCharacterRGBA[i][p].comp.a > inkA)
                            {
                            ...
                            if (y > 0 && x < leftExtreme)
                            {
                                ...
                                if (savedCharacterRGBA[i][pp].comp.a > inkA)
                                {

                                    leftExtreme = x;
                                }
                            }
                            if (y < mSpriteHeight - 1 && x < leftExtreme)
                            {
                    ...
                    for (int j = 0; j < 256; j++)
                    {
                        ...
                        if (savedCharacterRGBA[j] != NULL)
                        {
                            ...
                            for (int y = 0; y < mSpriteHeight; y++)
                            {
                                ...
                                int rowDistance = (mSpriteWidth - rightExtreme - 1) + leftExtreme;
                            ...
                            // of first character
                            minDistance -= mSpriteWidth - (mCharLeftEdgeOffset[i] + mCharWidth[i]);
...
{
    memcpy(mCharLeftEdgeOffset, inOtherFont->mCharLeftEdgeOffset, 256 * sizeof(int));

    memcpy(mCharWidth, inOtherFont->mCharWidth, 256 * sizeof(int));
    ...
    for (int i = 0; i < 256; i++)
    {
        ...
        {
            mKerningTable[i] = new KerningTable;
            memcpy(mKerningTable[i]->offset, inOtherFont->mKerningTable[i]->offset, 256 * sizeof(short));
        }
    }
...
double Font::getCharPos(SimpleVector<doublePair> *outPositions, const char *inString, doublePair inPosition,
                        TextAlignment inAlign)
{
    ...
    for (unsigned int i = 0; i < numChars; i++)
    {
        ...
        double charWidth = positionCharacter((unsigned char)(inString[i]), charPos, &drawPos);
        outPositions->push_back(drawPos);

        x += charWidth + mCharSpacing * scale;

        if (!mFixedWidth && mEnableKerning && i < numChars - 1 && mKerningTable[(unsigned char)(inString[i])] != NULL)
        {
            // there's another character after this
            // apply true kerning adjustment to the pair
            int offset = mKerningTable[(unsigned char)(inString[i])]->offset[(unsigned char)(inString[i + 1])];
            x += offset * scale;
        }
    }
...
double Font::positionCharacter(unsigned char inC, doublePair inTargetPos, doublePair *outActualPos)
{
...
double Font::measureString(const char *inString, int inCharLimit)
{
    ...
    for (int i = 0; i < numChars; i++)
    {
        ...
        else
        {
            ...
            if (mEnableKerning && i < numChars - 1 && mKerningTable[(unsigned char)(inString[i])] != NULL)
            {
                // there's another character after this
                // apply true kerning adjustment to the pair
                int offset = mKerningTable[(unsigned char)(inString[i])]->offset[(unsigned char)(inString[i + 1])];
