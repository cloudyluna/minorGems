...
class Font
{
  ...
  public:
    ...
    Font(const char *inFileName, int inCharSpacing, int inSpaceWidth, char inFixedWidth, double inScaleFactor = 1.0,
    ...
    double drawString(const char *inString, doublePair inPosition, TextAlignment inAlign = alignCenter);

    double measureString(const char *inString, int inCharLimit = -1);

    // gets per-character position of string without drawing it
    double getCharPos(SimpleVector<doublePair> *outPositions, const char *inString, doublePair inPosition,
  ...
  private:
    // returns x coordinate to right of drawn character
    double positionCharacter(unsigned char inC, doublePair inTargetPos, doublePair *outActualPos);
