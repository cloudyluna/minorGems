...
char startUpdate(char *inUpdateServerURL, int inOldVersionNumber)
{
    ...
    if (hgFileA.exists() || hgFileB.exists() || hgFileC.exists() || hgFileD.exists())
    {
    ...
    if (gitFileA.exists() || gitFileB.exists() || gitFileC.exists() || gitFileD.exists())
    {
    ...
    char *fullURL = autoSprintf("%s?action=is_update_available"
                                ...
                                inUpdateServerURL, platformCode, inOldVersionNumber);
...
static void dumpRawDataToFile(unsigned char *inRawBundleData, int inNumBytes)
{
...
static int applyUpdateFromWebResult()
{
    ...
    if (rawSize > 0 && compSize > 0)
    {
        ...
        unsigned char *rawData = zipDecompress(compData, compSize, rawSize);
        ...
        for (int d = 0; d < numRemovedFiles; d++)
        {
            ...
            int fileNameLength = scanIntAndSkip(&nextScanPointer, &success);
            ...
            nextScanPointer = &(nextScanPointer[fileNameLength + 1]);
        ...
        for (int f = 0; f < numFiles; f++)
        {
            ...
            // this skips it
            int fileSize = scanIntAndSkip(&nextScanPointer, &success);
            ...
            {
                int numWritten = fwrite((unsigned char *)nextScanPointer, 1, fileSize, file);
            ...
            if (currentUpdateUniversal && WINDOWS_LINE_ENDS && strstr(fileName, ".txt") != NULL)
            {
                ...
                if (contents != NULL)
                {
                    ...
                    if (strstr(contents, "\n") != NULL && strstr(contents, "\r\n") == NULL)
                    {
                        ...
                        char *convertedContents = newContents.getElementString();
        ...
        for (int i = 0; i < backupList.size(); i++)
        {
            ...
            if (remove(backName) != 0)
            {
                ...
                FILE *postRemoveListFile = fopen("postRemoveList.txt", "a");
                if (postRemoveListFile != NULL)
                {
                    fprintf(postRemoveListFile, "%s\n", backName);
    ...
    if (batchStepsDone < mirrors.size())
    {
        ...
        else
        {
            ...
            if (list->currentMirror < list->mirrorURLS.size())
            {
                ...
                if (strstr(list->mirrorURLS.getElementDirect(list->currentMirror), "_all.dbz") != NULL)
                {
                ...
                // start a request
                printf("Trying to fetch:  %s\n", list->mirrorURLS.getElementDirect(list->currentMirror));

                webHandle = startWebRequest("GET", list->mirrorURLS.getElementDirect(list->currentMirror), NULL);
        ...
        if (updateSize == -1)
        {
            ...
            else
            {
                ...
                char *fullURL = autoSprintf("%s?action=get_update"
                                            "&platform=%s&old_version=%d",
                                            updateServerURL, platformCode, oldVersionNumber);
