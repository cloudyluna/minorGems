...
static void bundleFileList(File **inFiles, int inNumFiles, SimpleVector<unsigned char> *inFileDataBuffer)
{

    char *fileCount = autoSprintf("%d ", inNumFiles);
    inFileDataBuffer->appendArray((unsigned char *)fileCount, strlen(fileCount));
    ...
    for (int i = 0; i < inNumFiles; i++)
    {
        ...
        char *header = autoSprintf("%d %s ", strlen(fileSubdirName), fileSubdirName);
        inFileDataBuffer->appendArray((unsigned char *)header, strlen(header));
        delete[] header;
...
static void bundleFiles(File **inFilesToRemove, int inNumFilesToRemove, File **inDirsToRemove, int inNumDirsToRemove,
                        File **inDirs, int inNumDirs, File **inFiles, int inNumFiles, char *inDBZTargetFile)
{
    ...
    char *fileCount = autoSprintf("%d ", inNumFiles);
    fileDataBuffer.appendArray((unsigned char *)fileCount, strlen(fileCount));
    ...
    for (int i = 0; i < inNumFiles; i++)
    {
        ...
        char *header = autoSprintf("%d %s %d#", strlen(fileSubdirName), fileSubdirName, size);
        fileDataBuffer.appendArray((unsigned char *)header, strlen(header));
        delete[] header;

        int contentLength;
        unsigned char *contents = inFiles[i]->readFileContents(&contentLength);
    ...
    */
    ...
    if (compData != NULL)
    {
        ...
        if (outFile != NULL)
        {
            ...
            printf("Wrote %d bytes to file %s\n", (int)ftell(outFile), inDBZTargetFile);
            fclose(outFile);
        }
        else
        {
...
static void printFileList(const char *inDescription, SimpleVector<File *> *inList)
{
    ...
    {
        char *fileName = (*inList->getElement(i))->getFullFileName();
...
int main(int inNumArgs, char **inArgs)
{
    ...
    if (inNumArgs == 5)
    {
        ...
        bundleFiles(NULL, 0, NULL, 0, newDirsArray, numNewDirs, newNonDirsArray, numNewNonDirs, inArgs[4]);
    ...
    for (int i = 0; i < numNewChild; i++)
    {
        ...
        int newFileLength;
        unsigned char *newFileContents = newChild[i]->readFileContents(&newFileLength);
        ...
        for (int j = 0; j < numOldChild && !foundOldFile; j++)
        {
            ...
            if (strcmp(oldFileSubdirName, newFileSubdirName) == 0)
            {
                ...
                if (!doneComparing)
                {
                    ...
                    int oldFileLength;
                    unsigned char *oldFileContents = oldChild[j]->readFileContents(&oldFileLength);
    ...
    bundleFiles(removedFilesArray, numRemovedFiles, removedDirsArray, numRemovedDirs, newDirsArray, numNewDirs,
