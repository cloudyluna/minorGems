...
void drawRect(double inStartX, double inStartY, double inEndX, double inEndY)
{

    double vertices[8] = {inStartX, inStartY, inStartX, inEndY, inEndX, inEndY, inEndX, inStartY};
...
{
    drawRect(inCenter.x - inRadius, inCenter.y - inRadius, inCenter.x + inRadius, inCenter.y + inRadius);
}

void drawRect(doublePair inCenter, double inHorizontalRadius, double inVerticalRadius)
{

    drawRect(inCenter.x - inHorizontalRadius, inCenter.y - inVerticalRadius, inCenter.x + inHorizontalRadius,
...
void drawCircle(doublePair inCenter, double inRadius)
{
    ...
    memcpy(workingCircleVerts, unitCircleVerts, sizeof(double) * NUM_UNIT_CRICLE_VERTS * 2);
    ...
    // draw as fan
    drawTriangles(NUM_UNIT_CIRCLE_TRIS, workingCircleVerts, false, true);
}

void drawCircle(doublePair inCenter, double inRadius, Color inCenterColor, Color inOuterColor)
{
    ...
    // draw as fan
    drawTrianglesColor(NUM_UNIT_CIRCLE_TRIS, workingCircleVerts, unitCircleColors, false, true);
}
