...
void drawTriangles(int inNumTriangles, double inVertices[], char inStrip = false, char inFan = false);

// four r,g,b,a values per triangle vertex
void drawTrianglesColor(int inNumTriangles, double inVertices[], float inVertexColors[], char inStrip = false,
                        char inFan = false);
...
void startAddingToStencil(char inDrawColorToo, char inAdd, float inMinAlpha = 0.0f);
...
SpriteHandle loadSprite(const char *inTGAFileName, char inTransparentLowerLeftCorner = true);

// same, but loads from the main directory
SpriteHandle loadSpriteBase(const char *inTGAFileName, char inTransparentLowerLeftCorner = true);

SpriteHandle fillSprite(Image *inImage, char inTransparentLowerLeftCorner = true);
...
void drawSprite(SpriteHandle inSprite, doublePair inCenter, double inZoom = 1.0, double inRotation = 0.0,
                char inFlipH = false);

// draw sprite with separate colors set for each corner
// corners in BL, BR, TR, TL order
void drawSprite(SpriteHandle inSprite, doublePair inCenter, FloatColor inCornerColors[4], double inZoom = 1.0,
                double inRotation = 0.0, char inFlipH = false);
...
// only it's alpha.
void drawSpriteAlphaOnly(SpriteHandle inSprite, doublePair inCenter, double inZoom = 1.0, double inRotation = 0.0,
                         char inFlipH = false);
