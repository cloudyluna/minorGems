...
DemoCodeChecker::DemoCodeChecker(char *inCode, const char *inSharedSecret, const char *inServerURL)
    : mSharedSecret(inSharedSecret), mServerURL(inServerURL), mError(false), mErrorString(NULL)
{

    char *challengeString = autoSprintf("%.f%s", Time::timeSec(), mSharedSecret);
    ...
    AppLog::getLog()->logPrintf(Log::INFO_LEVEL, "Checking demo code with URL %s", url);
...
char DemoCodeChecker::step()
{
    ...
    if (result == 1)
    {
        ...
        AppLog::getLog()->logPrintf(Log::INFO_LEVEL, "Result from demo code server:\n%s", resultString);

        if (strstr(resultString, "permitted") != NULL)
        {

            char *correctResponseString = autoSprintf("%s%s", mChallenge, mSharedSecret);

            char *correctResponse = computeSHA1Digest(correctResponseString);

            delete[] correctResponseString;

            char containsResponse = (strstr(resultString, correctResponse) != NULL);
    ...
    AppLog::getLog()->logPrintf(Log::ERROR_LEVEL, "Unexpecte result code from WebRequest: %d", result);
...
void DemoCodeChecker::setError(const char *inErrorTranslationKey)
{
    ...
    mErrorString = stringDuplicate(TranslationManager::translate((char *)inErrorTranslationKey));
}
