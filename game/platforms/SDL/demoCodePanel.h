...
static void setLabelString(LabelGL *inLabel, const char *inTranslationString, double inScaleFactor = 1)
{
    char *labelString = (char *)TranslationManager::translate((char *)inTranslationString);
    ...
    double actualDrawWidth = height * text->measureTextWidth(labelString);
    ...
    inLabel->setPosition(labelX, guiY, width, height);
}

static LabelGL *createLabel(double inGuiY, const char *inTranslationString, TextGL *inText = NULL)
{
    ...
    LabelGL *returnLabel = new LabelGL(0, inGuiY, 0, 0, "", inText);
    ...
    void keyPressed(unsigned char inKey, int inX, int inY)
    {
        ...
        if (inKey == 13 && enterDemoCodeField->isFocused())
        {
            ...
            if (strlen(enteredCode) > 0)
            {
                ...
                setLabelString(messageLabel, "checkingCode");

                // save this for next time
                SettingsManager::setSetting("demoCode", enteredCode);

                // start
                codeChecker = new DemoCodeChecker(enteredCode, getDemoCodeSharedSecret(), getDemoCodeServerURL());
            }
        }
...
static void panelCommonInit(ScreenGL *inScreen, const char *inFontTGAFileName, int inWidth, int inHeight)
{
    ...
    codeCheckerTextGLFixedWidth = new TextGL(fontImage,
                                             // use alpha
                                             true,
    ...
    codeCheckerTextGL = new TextGL(fontImage,
                                   // use alpha
                                   true,
    ...
    codeCheckerMainPanel = new GUIPanelGL(0, 0, codeCheckerWidth, codeCheckerWidth, new Color(0, 0, 0, 1.0));

    codeCheckerGuiTranslator = new GUITranslatorGL(codeCheckerMainPanel, codeCheckerScreen, codeCheckerWidth);
...
void showDemoCodePanel(ScreenGL *inScreen, const char *inFontTGAFileName, int inWidth, int inHeight)
{
    ...
    messageLabel = createLabel(0.5 * codeCheckerWidth + 15, "enterDemoCode");
    ...
    char *defaultCode = SettingsManager::getStringSetting("demoCode");
    ...
    enterDemoCodeField =
        new TextFieldGL(centerW - 0.5 * width, 0.5 * codeCheckerWidth - 15, width, height, 1, fieldDefault,
                        codeCheckerTextGLFixedWidth, new Color(0.75, 0.75, 0.75), new Color(0.75, 0.75, 0.75),
                        new Color(0.15, 0.15, 0.15), new Color(0.75, 0.75, 0.75), demoCodeLength, true);
...
char isDemoCodePanelShowing()
{
    ...
    if (codeChecker != NULL)
    {
        ...
        else
        {
            ...
            if (!checkerWorkLeft && !codeChecker->isError() && codeChecker->codePermitted())
            {
...
void showWriteFailedPanel(ScreenGL *inScreen, const char *inFontTGAFileName, int inWidth, int inHeight)
{
    ...
    messageLabel = createLabel(0.5 * codeCheckerWidth, "writeFailed");
    codeCheckerMainPanel->add(messageLabel);
}
