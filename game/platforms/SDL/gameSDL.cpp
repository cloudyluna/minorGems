...
void showWriteFailedPanel(ScreenGL *inScreen, const char *inFontTGAFileName, int inWidth, int inHeight)
{
}
...
class GameSceneHandler : public SceneHandlerGL,
...
void fadeSoundSprites(double inFadeSeconds)
{
    ...
    soundSpriteFadeIncrementPerSample = 1.0f / (inFadeSeconds * soundSampleRate);
...
SoundSpriteHandle loadSoundSprite(const char *inFolderName, const char *inAIFFFileName)
{
...
void setMaxTotalSoundSpriteVolume(double inMaxTotal, double inCompressionFraction)
{
    ...
    totalSoundSpriteNormalizeFactor = 1.0 / (1.0 - soundSpriteCompressionFraction);

    soundSpriteNoClip = resetAudioNoClip((1.0 - soundSpriteCompressionFraction) * maxTotalSoundSpriteVolume * 32767,
...
{
    if (soundSpriteRateMax != 1.0 || soundSpriteRateMin != 1.0)
    {

        return randSource.getRandomBoundedDouble(soundSpriteRateMin, soundSpriteRateMax);
    }
    else
    {
        return 1.0;
...
{
    if (soundSpriteVolumeMax != 1.0 || soundSpriteVolumeMin != 1.0)
    {

        return randSource.getRandomBoundedDouble(soundSpriteVolumeMin, soundSpriteVolumeMax);
    }
    else
    {
        return 1.0;
...
// no locking
static void playSoundSpriteInternal(SoundSpriteHandle inHandle, double inVolumeTweak, double inStereoPosition,
                                    double inForceVolume = -1, double inForceRate = -1)
{
    ...
    if (maxSimultaneousSoundSprites != -1 && playingSoundSpriteVolumesR.size() >= maxSimultaneousSoundSprites)
    {
...
void playSoundSprite(int inNumSprites, SoundSpriteHandle *inHandles, double *inVolumeTweaks, double *inStereoPositions)
{
    ...
    {
        playSoundSpriteInternal(inHandles[i], inVolumeTweaks[i], inStereoPositions[i], volume, rate);
    }
    unlockAudio();
}
...
void audioCallback(void *inUserData, Uint8 *inStream, int inLengthToFill)
{
    ...
    if (playingSoundSprites.size() > 0)
    {
        ...
        for (int i = 0; i < playingSoundSprites.size(); i++)
        {
            ...
            else
            {
                ...
                while (filled < numSamples && samplesPlayedF < spriteNumSamples - 1)
                {
                    ...
                    soundSpriteMixingBufferL[filled] += volumeL * sampleBlend;

                    soundSpriteMixingBufferR[filled] += volumeR * sampleBlend;
        ...
        audioNoClip(&soundSpriteNoClip, soundSpriteMixingBufferL, soundSpriteMixingBufferR, numSamples);
        ...
        for (int i = 0; i < numSamples; i++)
        {
            Sint16 lSample = (Sint16)((inStream[filledBytes + 1] << 8) | inStream[filledBytes]);
            Sint16 rSample = (Sint16)((inStream[filledBytes + 3] << 8) | inStream[filledBytes + 2]);
        ...
        audioNoClip(&totalAudioMixNoClip, soundSpriteMixingBufferL, soundSpriteMixingBufferR, numSamples);
        ...
        for (int i = playingSoundSprites.size() - 1; i >= 0; i--)
        {
            ...
            if (soundSpriteGlobalLoudness == 0 || s->samplesPlayed >= s->numSamples ||
    ...
    if ((currentSoundLoudness != soundLoudness && !sceneHandler->mPaused) ||
        (currentSoundLoudness != 0.0f && sceneHandler->mPaused) || currentSoundLoudness != 1.0f)
    {

        int nextByte = 0;
        for (int i = 0; i < numSamples; i++)
        {
            Sint16 lSample = inStream[nextByte] | (inStream[nextByte + 1] << 8);

            Sint16 rSample = inStream[nextByte + 2] | (inStream[nextByte + 3] << 8);
            ...
            else if (currentSoundLoudness != 0 && sceneHandler->mPaused)
            {
...
void handleTooWide()
{
    if (isNonIntegerScalingAllowed())
    {
        ...
        if (screenRatio > gameRatio)
        {
            ...
            AppLog::infoF("Screen has wider aspect ratio than desired game image, "
...
static char *pickFolder(const char *inPrompt)
{
    ...
    const char *commandFormat = "osascript -e 'tell application \"Finder\" to activate' "
    ...
    {
        AppLog::error("Failed to open pipe to osascript for picking a folder.");
    }
    else
    {
        char buffer[200];
...
static void showMessage(const char *inAppName, const char *inTitle, const char *inMessage, char inError = false)
{
    ...
    const char *commandFormat = "osascript -e 'tell app \"Finder\" to activate' "
                                "-e 'tell app \"Finder\" to display dialog \"%s\" "
                                "with title \"%s:  %s\" buttons \"Ok\" "
                                "with icon %s default button \"Ok\"' ";

    char *command = autoSprintf(commandFormat, inMessage, inAppName, inTitle, iconName);
...
{
    return autoSprintf("%s/Library/Preferences/%s_prefs.txt", getenv("HOME"), getAppName());
}
...
// change the current working directory out from under us)
    ...
    if (!isSettingsFolderFound())
    {
        ...
        if (path != NULL)
        {
            ...
            if (prefFilePath == NULL)
            {
                ...
                showMessage(getAppName(), "First Start Up Error", message, true);
                delete[] message;
            }
            else
            {
            ...
            if (!isSettingsFolderFound())
            {
                showMessage(getAppName(), "First Start Up Error", "Still cannot find game data, exiting.", true);
                return 1;
            }
        }
        else
        {
            showMessage(getAppName(), "First Start Up Error", "Picking a folder failed, exiting.", true);
            return 1;
        }
    }
    ...
    if (!fullscreen && useLargestWindow)
    {
        ...
        if (isNonIntegerScalingAllowed())
        {
            ...
            else if (tryW > screenWidth)
            {
                ...
                {
                    AppLog::getLog()->logPrintf(Log::INFO_LEVEL, "Largest-window mode would offer a <25% width "
        ...
        else
        {
            ...
            while (gameWidth * blowUpFactor < currentW && gameHeight * blowUpFactor < currentH)
            {
            ...
            while (blowUpFactor > 1 &&
                   (gameWidth * blowUpFactor >= currentW * 0.85 || gameHeight * blowUpFactor >= currentH * 0.85))
            {
        ...
        AppLog::getLog()->logPrintf(Log::INFO_LEVEL, "Screen dimensions for largest-window mode:  %dx%d\n", screenWidth,
                                    screenHeight);
    }
    else if (!fullscreen && !useLargestWindow)
    {
        ...
        if (isNonIntegerScalingAllowed() && (screenWidth > currentW || screenHeight > currentH))
        {
            double aspectRatio = screenHeight / (double)screenWidth;
        ...
        else
        {
            ...
            while (screenWidth / blowDownFactor > currentW || screenHeight / blowDownFactor > currentH)
            {
                blowDownFactor += 1;
            }
    ...
    screen = new ScreenGL(screenWidth, screenHeight, fullscreen, shouldNativeScreenResolutionBeUsed(), targetFrameRate,
                          recordGame, customData, hashSalt, getWindowTitle(), NULL, NULL, NULL);
    ...
    AppLog::infoF("ScreenGL gave us %dx%d, %d fps", screenWidth, screenHeight, targetFrameRate);
    ...
    int forceBigPointer = SettingsManager::getIntSetting("forceBigPointer", 0);
    if (forceBigPointer || screenWidth > 1920 || screenHeight > 1080)
    {
        // big cursor
        ...
        if (cursorImage != NULL)
        {
            ...
            if (cursorImage->getWidth() == 32 && cursorImage->getHeight() == 32 && cursorImage->getNumChannels() == 4)
            {
                ...
                ourCursor = SDL_CreateCursor(data, mask, 32, 32, 0, 0);

                SDL_SetCursor(ourCursor);
            }
            else
            {
                AppLog::error("bigPointer.tga is not a 32x32 4-channel image.");
            }
    ...
    */
    ...
    if (!isNonIntegerScalingAllowed())
    {
        ...
        screen->setImageSize(pixelZoomFactor * gameWidth, pixelZoomFactor * gameHeight);

        AppLog::infoF("Setting integer-scaled screen image size of %dx%d", pixelZoomFactor * gameWidth,
    ...
    else
    {
        ...
        screen->setImageSize(imageH, imageW);

        AppLog::infoF("Setting non-integer-scaled screen image size of %dx%d", imageW, imageH);
    }
    ...
    if (getUsesSound())
    {
        ...
        int recordAudioFlag = SettingsManager::getIntSetting("recordAudio", 0);
        int recordAudioLengthInSeconds = SettingsManager::getIntSetting("recordAudioLengthInSeconds", 0);
        ...
        if (openResult < 0)
        {
            AppLog::getLog()->logPrintf(Log::ERROR_LEVEL, "Unable to open audio: %s\n", SDL_GetError());
            soundRunning = false;
            soundOpen = false;
        }
        else
        {
            ...
            if (!recordAudioFlag && (actualFormat.format != AUDIO_S16 || actualFormat.channels != 2))
            {

                AppLog::getLog()->logPrintf(Log::ERROR_LEVEL, "Able to open audio, "
            ...
            else
            {
                ...
                if (!recordAudioFlag)
                {
                    AppLog::getLog()->logPrintf(Log::INFO_LEVEL,
                                                "Successfully opened audio: %dHz (requested %dHz), "
                                                "sample buffer size=%d (requested %d)\n",
                                                actualFormat.freq, desiredRate, actualFormat.samples, bufferSize);
                ...
                soundSpriteNoClip =
                    resetAudioNoClip((1.0 - soundSpriteCompressionFraction) * maxTotalSoundSpriteVolume * 32767,
                                     // half second hold and release
                                     soundSampleRate / 2, soundSampleRate / 2);

                totalAudioMixNoClip = resetAudioNoClip(32767.0,
                                                       ...
                                                       soundSampleRate / 20, soundSampleRate / 20);
                ...
                if (recordAudioFlag == 1 && recordAudioLengthInSeconds > 0)
                {
                    ...
                    samplesLeftToRecord = recordAudioLengthInSeconds * soundSampleRate;

                    aiffOutFile = fopen("recordedAudio.aiff", "wb");

                    int headerLength;

                    unsigned char *header = getAIFFHeader(2, 16, soundSampleRate, samplesLeftToRecord, &headerLength);
...
GameSceneHandler::GameSceneHandler(ScreenGL *inScreen)
    : mScreen(inScreen), mPaused(false), mPausedDuringFrameBatch(true), mLoadingDuringFrameBatch(true),
      mPausedSleepTime(0), mBlockQuitting(false), mLastFrameRate(targetFrameRate), mStartTimeSeconds(Time::timeSec()),
      mPrintFrameRate(true), mNumFrames(0), mFrameBatchSize(100), mFrameBatchStartTimeSeconds(-1),
...
static void redoDrawMatrix()
{
    ...
    glLoadIdentity();
    glOrtho(viewCenterX - wRadius, viewCenterX + wRadius, viewCenterY - hRadius, viewCenterY + hRadius, -1.0f, 1.0f);
    ...
    if (visibleHeight > 0)
    {
        ...
        glViewport(excessW / 2, excessH / 2, portWide, portHigh);
    }
...
void setLetterbox(float inVisibleWidth, float inVisibleHeight)
{
    ...
    AppLog::infoF("setLetterbox called with %fx%f", inVisibleWidth, inVisibleHeight);
    redoDrawMatrix();
}
    ...
    */
    ...
    else if (!screen->isPlayingBack() && measureFrameRate)
    {
        ...
        else
        {
            ...
            {
                char *message = autoSprintf("%s\n%0.2f\nFPS", translate("measuringFPS"), frameRate);
            ...
            if (totalTime > secondsToMeasure)
            {
                ...
                if (!measureRecorded)
                {
                    ...
                    if (targetFrameRate == idealTargetFrameRate)
                    {
                        ...
                        if (frameRate > overAllowFactor * closestTargetFrameRate)
                        {
                            ...
                            AppLog::infoF("Vsync to enforce closested frame rate of "
                        ...
                        {
                            AppLog::infoF("Vsync seems to be enforcing an allowed frame "
                                          "rate of %d fps.\n",
                                          closestTargetFrameRate);
                    ...
                    else
                    {
                        ...
                        AppLog::infoF("User has halfFrameRate set, so we're going "
                ...
                if (!countingOnVsync)
                {
                    ...
                    char *message =
                        autoSprintf("%s\n%s\n\n%s\n\n\n%s", translate("vsyncWarning"), translate("vsyncWarning2"),
                                    translate("vsyncWarning3"), translate("vsyncContinueMessage"));
                    drawString(message, true);
    ...
    else if (!writeFailed && !loadingFailedFlag && !frameDrawerInited)
    {
        ...
        initFrameDrawer(pixelZoomFactor * gameWidth, pixelZoomFactor * gameHeight, targetFrameRate,
                        screen->getCustomRecordedGameData(), screen->isPlayingBack());
        ...
        else
        {
            ...
            double readCursorScale = SettingsManager::getDoubleSetting("emulatedCursorScale", -1.0);
    ...
    else if (!writeFailed && !loadingFailedFlag)
    {
        ...
        if (cursorMode > 0)
        {
            ...
            double vertsA[18] = {// body of pointer
                                 x, y, x, y - sizeFactor * 0.8918, x + sizeFactor * 0.6306, y - sizeFactor * 0.6306,
                                 // left collar of pointer
                                 x, y, x, y - sizeFactor * 1.0, x + sizeFactor * 0.2229, y - sizeFactor * 0.7994,
                                 // right collar of pointer
                                 x + sizeFactor * 0.4077, y - sizeFactor * 0.7229, x + sizeFactor * 0.7071,
                                 y - sizeFactor * 0.7071, x, y};

            drawTriangles(3, vertsA);

            // neck of pointer
            double vertsB[8] = {x + sizeFactor * 0.2076, y - sizeFactor * 0.7625, x + sizeFactor * 0.376,
                                y - sizeFactor * 1.169,  x + sizeFactor * 0.5607, y - sizeFactor * 1.0924,
            ...
            double vertsC[18] = {// body of pointer
                                 x + sizeFactor * 0.04, y - sizeFactor * 0.0966, x + sizeFactor * 0.04,
                                 y - sizeFactor * 0.814, x + sizeFactor * 0.5473, y - sizeFactor * 0.6038,
                                 // left collar of pointer
                                 x + sizeFactor * 0.04, y - sizeFactor * 0.0966, x + sizeFactor * 0.04,
                                 y - sizeFactor * 0.9102, x + sizeFactor * 0.2382, y - sizeFactor * 0.7319,
                                 // right collar of pointer
                                 x + sizeFactor * 0.3491, y - sizeFactor * 0.6859, x + sizeFactor * 0.6153,
                                 y - sizeFactor * 0.6719, x + sizeFactor * 0.04, y - sizeFactor * 0.0966};

            drawTriangles(3, vertsC);

            // neck of pointer
            double vertsD[8] = {x + sizeFactor * 0.2229, y - sizeFactor * 0.6949, x + sizeFactor * 0.3976,
                                y - sizeFactor * 1.1167, x + sizeFactor * 0.5086, y - sizeFactor * 1.0708,
        ...
        if (screen->isPlayingBack() && screen->shouldShowPlaybackDisplay())
        {
            ...
            char *progressString =
                autoSprintf("%s %.1f\n%s\n%s", translate("playbackTag"), screen->getPlaybackDoneFraction() * 100,
                            translate("playbackToggleMessage"), translate("playbackEndMessage"));
        ...
        if (screen->isPlayingBack() && screen->shouldShowPlaybackDisplay() && showMouseDuringPlayback())
        {
            ...
            glOrtho(-excessX, -excessX + bigDimension, -excessY + bigDimension, -excessY, -1.0f, 1.0f);

            glViewport(-excessX, -excessY, bigDimension, bigDimension);

            glMatrixMode(GL_MODELVIEW);

            double verts[8] = {lastMouseX - sizeFactor, lastMouseY - sizeFactor, lastMouseX - sizeFactor,
                               lastMouseY + sizeFactor, lastMouseX + sizeFactor, lastMouseY + sizeFactor,
            ...
            if (showClick)
            {
                double clickVerts[8] = {lastMouseDownX - clickSizeFactor, lastMouseDownY - clickSizeFactor,
                                        lastMouseDownX - clickSizeFactor, lastMouseDownY + clickSizeFactor,
                                        lastMouseDownX + clickSizeFactor, lastMouseDownY + clickSizeFactor,
                                        lastMouseDownX + clickSizeFactor, lastMouseDownY - clickSizeFactor};
                ...
                double clickCenterVerts[8] = {lastMouseDownX - centerSize, lastMouseDownY - centerSize,
                                              lastMouseDownX - centerSize, lastMouseDownY + centerSize,
                                              lastMouseDownX + centerSize, lastMouseDownY + centerSize,
                                              lastMouseDownX + centerSize, lastMouseDownY - centerSize};
            ...
            double centerVerts[8] = {lastMouseX - centerSize, lastMouseY - centerSize, lastMouseX - centerSize,
                                     lastMouseY + centerSize, lastMouseX + centerSize, lastMouseY + centerSize,
    ...
    if (visibleWidth != -1 && visibleHeight != -1)
    {
        ...
        int extraWidth = lrint(screenWidth - ((double)visibleWidth / (double)visibleHeight) * screenHeight);

        if (extraWidth > 0)
        {

            // left/right bars
            drawRect(0, 0, extraWidth / 2, screenHeight);

            drawRect(screenWidth - extraWidth / 2, 0, screenWidth, screenHeight);
        }
        else
        {

            int extraHeight = lrint(screenHeight - ((double)visibleHeight / (double)visibleWidth) * screenWidth);
            ...
            if (extraHeight > 0)
            {
                ...
                drawRect(0, 0, screenWidth, extraHeight / 2);

                drawRect(0, screenHeight - extraHeight / 2, screenWidth, screenHeight);
            }
        }
    }
...
void GameSceneHandler::keyReleased(unsigned char inKey, int inX, int inY)
{
...
void GameSceneHandler::specialKeyPressed(int inKey, int inX, int inY)
{
...
void GameSceneHandler::specialKeyReleased(int inKey, int inX, int inY)
{
...
char isCommandKeyDown()
{
    ...
    if ((modState & KMOD_CTRL) || (modState & KMOD_ALT) || (modState & KMOD_META))
    {
    ...
    if (screen->isPlayingBack())
    {
        ...
        if (rCtrlDown || lCtrlDown || rAltDown || lAltDown || rMetaDown || lMetaDown)
        {
            return true;
        }
    }
...
static Image *readTGAFile(File *inFile)
{
    ...
    if (!inFile->exists())
    {
        ...
        char *logString = autoSprintf("CRITICAL ERROR:  TGA file %s does not exist", fileName);
        delete[] fileName;
    ...
    if (result == NULL)
    {
        ...
        char *logString = autoSprintf("CRITICAL ERROR:  could not read TGA file %s, wrong format?", fileName);
        delete[] fileName;
...
SpriteHandle fillSprite(RawRGBAImage *inRawImage)
{
    ...
    return fillSprite(inRawImage->mRGBABytes, inRawImage->mWidth, inRawImage->mHeight);
}

SpriteHandle loadSprite(const char *inTGAFileName, char inTransparentLowerLeftCorner)
{
    ...
    else
    {
        ...
        SpriteHandle sprite = fillSprite(result, inTransparentLowerLeftCorner);
...
static Image *getScreenRegionInternal(int inStartX, int inStartY, int inWidth, int inHeight, char inForceManual = false)
{
    ...
    glReadPixels(inStartX, inStartY, inWidth, inHeight, GL_RGB, GL_UNSIGNED_BYTE, rgbBytes);

    glPixelStorei(GL_PACK_ALIGNMENT, oldAlignment);

    if (!inForceManual && !manualScreenShot && blendOutputFramePairs && frameNumber % 2 != 0 &&
        lastFrame_rgbaBytes != NULL)
    {
        ...
        if (blendOutputFrameFraction > 0)
        {
            ...
            {
                rgbBytes[i] = (unsigned char)(blendA * rgbBytes[i] + blendB * lastFrame_rgbaBytes[i]);
            }
        }
    }
    else if (!inForceManual && !manualScreenShot && blendOutputFramePairs && frameNumber % 2 == 0)
    {
    ...
    for (int y = inHeight - 1; y >= 0; y--)
    {
        for (int x = 0; x < inWidth; x++)
        {
            ...
            channelOne[outputPixelIndex] = rgbBytes[byteIndex++] * 0.003921569;
            channelTwo[outputPixelIndex] = rgbBytes[byteIndex++] * 0.003921569;
            channelThree[outputPixelIndex] = rgbBytes[byteIndex++] * 0.003921569;
        }
        outputRow++;
    }
...
void takeScreenShot()
{
    ...
    if (nextShotNumber < 1)
    {
        if (shotDir.exists() && shotDir.isDirectory())
        {
            ...
            char *formatString = autoSprintf("%s%%d.%s", screenShotPrefix, screenShotExtension);
    ...
    char *fileName = autoSprintf("%s%05d.%s", screenShotPrefix, nextShotNumber, screenShotExtension);
    ...
    if (outputAllFrames)
    {
        printf("Output Frame %d (%.2f sec)\n", outputFrameCount, outputFrameCount / (double)targetFrameRate);
        outputFrameCount++;
    }

    Image *screenImage = getScreenRegionInternal(0, 0, screenWidth, screenHeight);
...
Image *getScreenRegion(double inX, double inY, double inWidth, double inHeight)
{
    ...
    gluProject(inX, inY, 0, modelview, projection, viewport, &winStartX, &winStartY, &winStartZ);

    gluProject(endX, endY, 0, modelview, projection, viewport, &winEndX, &winEndY, &winEndZ);

    char oldManual = manualScreenShot;
    manualScreenShot = true;

    Image *result = getScreenRegionInternal(lrint(winStartX), lrint(winStartY), lrint(winEndX - winStartX),
                                            lrint(winEndY - winStartY));
...
int startWebRequest(const char *inMethod, const char *inURL, const char *inBody)
{
...
unsigned char *getWebResult(int inHandle, int *outSize)
{
    if (screen->isPlayingBack())
    {
        ...
        if (nextType == 2)
        {
            return (unsigned char *)screen->getWebEventResultBody(inHandle, outSize);
        }
        else
        {
            AppLog::error("Expecting a web result body in playback file, "
...
int getWebProgressSize(int inHandle)
{
    if (screen->isPlayingBack())
    {
        ...
        {
            AppLog::error("Expecting a web result progress event in playback file, "
                          "but found none.");
...
int readFromSocket(int inHandle, unsigned char *inDataBuffer, int inBytesToRead)
{
    ...
    if (screen->isPlayingBack())
    {
        ...
        if (nextType == 2)
        {
            ...
            unsigned char *bodyBytes = screen->getSocketEventBodyBytes(inHandle);
...
int startAsyncFileRead(const char *inFilePath)
{
    ...
    AsyncFileRecord r = {handle, stringDuplicate(inFilePath), -1, NULL, false};
...
char checkAsyncFileReadDone(int inHandle)
{
    ...
    for (int i = 0; i < asyncFiles.size(); i++)
    {
        ...
        if (r->handle == inHandle && r->doneReading)
        {
...
void launchURL(char *inURL)
{
    ...
    char *call = autoSprintf("wmic process call create 'cmd /c start \"\" \"%s\"'", inURL);
    system(call);
    delete[] call;
}
...
char relaunchGame()
{
    char *launchTarget = autoSprintf("./%s", getLinuxAppName());
...
{
    char *launchTarget = autoSprintf("./%s", steamGateClientName);
...
char relaunchGame()
{
    char *launchTarget = autoSprintf("%s%s", getAppName(), winLaunchExtension);
...
char runSteamGateClient()
{
    char *launchTarget = autoSprintf("%s%s", steamGateClientName, winLaunchExtension);
...
char startRecording16BitMonoSound(int inSampleRate)
{
    ...
    char *arecordLine = autoSprintf("arecord -f S16_LE -c1 -r%d %s", inSampleRate, arecordFileName);
...
char startRecording16BitMonoSound(int inSampleRate)
{
    ...
    if (mciSendString("open new type waveaudio alias my_sound", NULL, 0, 0) == 0)
    {

        char *settingsString = autoSprintf("set my_sound alignment 2 bitspersample 16"
                                           " samplespersec %d"
                                           " channels 1"
                                           " bytespersec %d"
                                           " time format milliseconds format tag pcm",
                                           inSampleRate, (16 * inSampleRate) / 8);
...
int16_t *load16BitMonoSound(int *outNumSamples, int *outSampleRate)
{
    ...
    // little endian
    *outSampleRate = (int)(readBuffer[3] << 24 | readBuffer[2] << 16 | readBuffer[1] << 8 | readBuffer[0]);
    ...
    *outNumSamples = numSampleBytes / 2;
    ...
    {
        // little endian
        returnSamples[i] = (rawSamples[r + 1] << 8) | rawSamples[r];
...
void printImage(Image *inImage, char inFullColor)
{
    ...
    char *command = autoSprintf("convert -density 72x72 "
                                " %s %s ps:- | lpr",
