...
void SpriteGL::findColoredRadii(unsigned char *inRGBA, int inWidth, int inHeight)
{
...
void SpriteGL::findColoredRadiiAlpha(unsigned char *inA, int inWidth, int inHeight)
{
...
SpriteGL::SpriteGL(Image *inImage, char inTransparentLowerLeftCorner, int inNumFrames, int inNumPages,
                   char inSetColoredRadii)
{

    initTexture(inImage, inTransparentLowerLeftCorner, inNumFrames, inNumPages, inSetColoredRadii);
}

void SpriteGL::initTexture(Image *inImage, char inTransparentLowerLeftCorner, int inNumFrames, int inNumPages,
                           char inSetColoredRadii)
{
    ...
    if (inTransparentLowerLeftCorner)
    {
        ...
        else
        {
            ...
            int numPixels = spriteImage->getWidth() * spriteImage->getHeight();
...
SpriteGL::SpriteGL(unsigned char *inRGBA, unsigned int inWidth, unsigned int inHeight, int inNumFrames, int inNumPages,
                   char inSetColoredRadii)
{
...
SpriteGL::SpriteGL(char inAlphaOnly, unsigned char *inA, unsigned int inWidth, unsigned int inHeight, int inNumFrames,
                   int inNumPages, char inSetColoredRadii)
{
    ...
    mTexture = new SingleTextureGL(inAlphaOnly, inA, inWidth, inHeight,
                                   // no wrap
                                   false, sGenerateMipMaps);
...
void SpriteGL::setWrapping(char inHorizontal, char inVertical)
{
...
void SpriteGL::prepareDraw(int inFrame, Vector3D *inPosition, double inScale, char inLinearMagFilter,
                           char inMipMapFilter, double inRotation, char inFlipH, char inComputeCornerPos)
{
    /*
    ...
    */
    ...
    if (inComputeCornerPos)
    {
        ...
        float xRightRadius = (float)(inScale * mBaseScaleX * mColoredRadiusRightX);

        float yTopRadius = (float)(inScale * mBaseScaleY * mColoredRadiusTopY);
        float yBottomRadius = (float)(inScale * mBaseScaleY * mColoredRadiusBottomY);
...
void SpriteGL::draw(int inFrame, Vector3D *inPosition, double inScale, char inLinearMagFilter, char inMipMapFilter,
                    double inRotation, char inFlipH)
{

    prepareDraw(inFrame, inPosition, inScale, inLinearMagFilter, inMipMapFilter inRotation, inFlipH);
...
void SpriteGL::draw(int inFrame, Vector3D *inPosition, FloatColor inCornerColors[4], double inScale,
                    char inLinearMagFilter, char inMipMapFilter, double inRotation, char inFlipH)
{
...
void SpriteGL::draw(int inFrame, doublePair inCornerPos[4], FloatColor inCornerColors[4], char inLinearMagFilter,
                    char inMipMapFilter)
{

    prepareDraw(inFrame, &dummyPosition, 1, inLinearMagFilter, inMipMapFilter, 0, false,
...
void SpriteGL::draw(int inFrame, doublePair inCornerPos[4], doublePair inTexCoords[4], char inLinearMagFilter,
                    char inMipMapFilter)
{
