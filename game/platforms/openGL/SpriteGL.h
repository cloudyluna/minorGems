...
class SpriteGL
{
  public:
    ...
    SpriteGL(Image *inImage, char inTransparentLowerLeftCorner = false, int inNumFrames = 1, int inNumPages = 1,
             char inSetColoredRadii = false);

    SpriteGL(unsigned char *inRGBA, unsigned int inWidth, unsigned int inHeight, int inNumFrames = 1,
             int inNumPages = 1, char inSetColoredRadii = false);

    // one-channel, alpha-only, with other channels black
    // extra parameter just to differentiate function calls
    SpriteGL(char inAlphaOnly, unsigned char *inA, unsigned int inWidth, unsigned int inHeight, int inNumFrames = 1,
             int inNumPages = 1, char inSetColoredRadii = false);

    ~SpriteGL();

    void draw(int inFrame, Vector3D *inPosition, double inScale = 1, char inLinearMagFilter = false,
              char inMipMapFilter = false,
              // rotation where 0.25 = 90 degrees clockwise,
              // and 1.0 is a full  rotation
              double inRotation = 0, char inFlipH = false);

    // draw sprite with separate colors set for each corner
    // corners in BL, BR, TR, TL order
    void draw(int inFrame, Vector3D *inPosition, FloatColor inCornerColors[4], double inScale = 1,
              char inLinearMagFilter = false, char inMipMapFilter = false, double inRotation = 0, char inFlipH = false);

    // draw sprite on a quad with particular corner positions
    // (allows for sprite distortion)
    void draw(int inFrame, doublePair inCornerPos[4], FloatColor inCornerColors[4], char inLinearMagFilter = false,
              char inMipMapFilter = false);

    // FOVMOD NOTE:  Change 1/3 - Take these lines during the merge process
    void draw(int inFrame, doublePair inCornerPos[4], doublePair inTexCoords[4], char inLinearMagFilter = false,
              char inMipMapFilter = false);
    ...
    void setWrapping(char inHorizontal, char invertical);
  ...
  protected:
    ...
    void initTexture(Image *inImage, char inTransparentLowerLeftCorner = false, int inNumFrames = 1, int inNumPages = 1,
                     char inSetColoredRadii = false);

    // common code for two different drawing functions
    void prepareDraw(int inFrame, Vector3D *inPosition, double inScale, char inLinearMagFilter, char inMipMapFilter,
                     double inRotation, char inFlipH, char inComputeCornerPos = true,
    ...
    void findColoredRadii(unsigned char *inRGBA, int inWidth, int inHeight);

    void findColoredRadiiAlpha(unsigned char *inA, int inWidth, int inHeight);
};
