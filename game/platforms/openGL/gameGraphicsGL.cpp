...
static void recalcGlobalFade()
{
    ...
    {
        globalFadeTotal *= globalFades.getElementDirect(i).fade;
    }
}
...
void setDrawColor(float inR, float inG, float inB, float inA)
{
    ...
    if (additiveTextureColorMode && globalFadeTotal < 1.0f)
    {
...
FloatColor getFloatColor(const char *inHexString)
{
    ...
    FloatColor f = {r / 255.0f, g / 255.0f, b / 255.0f, 1.0f};
...
void drawQuads(int inNumQuads, double inVertices[], float inVertexColors[])
{
    ...
    drawTrianglesColor(inNumQuads * 2, triangleVertices, triangleVertexColors);
    delete[] triangleVertices;
    delete[] triangleVertexColors;
}

void drawTriangles(int inNumTriangles, double inVertices[], char inStrip, char inFan)
{
...
void drawTrianglesColor(int inNumTriangles, double inVertices[], float inVertexColors[], char inStrip, char inFan)
{
...
void enableScissor(double inX, double inY, double inWidth, double inHeight)
{
    ...
    glScissor(lrint(winStartX), lrint(winStartY), lrint(winEndX - winStartX), lrint(winEndY - winStartY));
...
SpriteHandle fillSprite(Image *inImage, char inTransparentLowerLeftCorner)
{
    ...
    return new SpriteGL(inImage, inTransparentLowerLeftCorner, 1, 1, transparentCroppingOn);
}
...
SpriteHandle fillSprite(unsigned char *inRGBA, unsigned int inWidth, unsigned int inHeight)
{
    ...
    return new SpriteGL(inRGBA, inWidth, inHeight, 1, 1, transparentCroppingOn);
}

SpriteHandle fillSpriteAlphaOnly(unsigned char *inA, unsigned int inWidth, unsigned int inHeight)
{

    return new SpriteGL(true, inA, inWidth, inHeight, 1, 1, transparentCroppingOn);
}
...
void drawSprite(SpriteHandle inSprite, doublePair inCenter, double inZoom, double inRotation, char inFlipH)
{
    ...
    sprite->draw(0, &spritePos, inZoom, linearTextureFilterOn, mipMapTextureFilterOn, inRotation, inFlipH);

    numSpritesDrawn++;
}

void drawSprite(SpriteHandle inSprite, doublePair inCenter, FloatColor inCornerColors[4], double inZoom,
                double inRotation, char inFlipH)
{
    ...
    sprite->draw(0, &spritePos, inCornerColors, inZoom, linearTextureFilterOn, mipMapTextureFilterOn, inRotation,
                 inFlipH);
...
void drawSprite(SpriteHandle inSprite, doublePair inCornerPos[4], FloatColor inCornerColors[4])
{
    ...
    sprite->draw(0, inCornerPos, inCornerColors, linearTextureFilterOn, mipMapTextureFilterOn);

    numSpritesDrawn++;
}

// FOVMOD NOTE:  Change 2/2 - Take these lines during the merge process
void drawSprite(SpriteHandle inSprite, doublePair inCornerPos[4], doublePair inTexCoords[4])
{
    SpriteGL *sprite = (SpriteGL *)inSprite;

    sprite->draw(0, inCornerPos, inTexCoords, linearTextureFilterOn, mipMapTextureFilterOn);

    numSpritesDrawn++;
}

void drawSpriteAlphaOnly(SpriteHandle inSprite, doublePair inCenter, double inZoom, double inRotation, char inFlipH)
{
