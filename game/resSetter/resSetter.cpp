...
int main()
{
    ...
    if (launchName == NULL)
    {
        AppLog::error("Failed to read app name from resSetterLaunch.txt, quitting.");
        return 1;
    }
    ...
    if (settingsFolder.exists() && settingsFolder.isDirectory())
    {
        ...
        int manualRes = SettingsManager::getIntSetting("manualScreenResolution", &found);
        ...
        else
        {
            ...
            AppLog::infoF("Found native resolution %d x %d", nativeX, nativeY);
            ...
            double mouseSpeed = SettingsManager::getDoubleSetting("mouseSpeed", &found);

            if (!found)
            {
                AppLog::infoF("No mouseSpeed settings file, leaving it unset", mouseSpeed);
            }
            else if (mouseSpeed == 1.0)
            {
                AppLog::info("Default mouseSpeed of 1.0 found");

                double newMouseSpeed = (double)defaultWidth / (double)nativeX;

                AppLog::infoF("Setting new mouse speed based on "
                              "screen width to %f",
                              newMouseSpeed);

                SettingsManager::setDoubleSetting("mouseSpeed", newMouseSpeed);
            }
            else
            {
