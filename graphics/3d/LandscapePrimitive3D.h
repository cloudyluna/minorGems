...
class LandscapePrimitive3D : public Primitive3D
{
  ...
  public:
     ...
     *   mesh.
     */
    LandscapePrimitive3D(int inWide, int inHigh, double *inHeights, RGBAImage *inTexture,
                         RGBAImage *inDetailTexture = NULL, double inDetailScale = 1.0);
};

inline LandscapePrimitive3D::LandscapePrimitive3D(int inWide, int inHigh, double *inHeights, RGBAImage *inTexture,
                                                  RGBAImage *inDetailTexture, double inDetailScale)
{
    ...
    for (int y = 0; y < mHigh; y++)
    {
        for (int x = 0; x < mWide; x++)
        {
            ...
            mVertices[index] = new Vector3D(2 * mAnchorX[0][index] - 1, inHeights[index], -2 * mAnchorY[0][index] + 1);
