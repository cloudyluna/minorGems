...
class Primitive3D : public Serializable
{
  ...
  public:
     ...
     *   in range [0,1] (outside this range, texture will wrap).
     */
    Primitive3D(long inWide, long inHigh, Vector3D **inVertices, long inNumTextures, RGBAImage **inTexture,
...
inline Primitive3D::Primitive3D(long inWide, long inHigh, Vector3D **inVertices, long inNumTextures,
                                RGBAImage **inTexture, double **inAnchorX, double **inAnchorY)
    : mHigh(inHigh), mWide(inWide), mNumVertices(inHigh * inWide), mNumTextures(inNumTextures), mVertices(inVertices),
      mTexture(inTexture), mAnchorX(inAnchorX), mAnchorY(inAnchorY), mTransparent(false), mBackVisible(false),
...
inline Primitive3D::Primitive3D() : mTransparent(false), mBackVisible(false), mMembersAllocated(false)
{
}
...
inline Primitive3D *Primitive3D::copy()
{
    ...
    for (i = 0; i < mNumTextures; i++)
    {
        ...
        memcpy(primCopy->mAnchorX[i], mAnchorX[i], sizeof(double) * mNumVertices);
        memcpy(primCopy->mAnchorY[i], mAnchorY[i], sizeof(double) * mNumVertices);
    }
...
inline int Primitive3D::serialize(OutputStream *inOutputStream)
{
    ...
    numBytes += inOutputStream->write((unsigned char *)&mTransparent, 1);

    numBytes += inOutputStream->write((unsigned char *)&mBackVisible, 1);
...
inline int Primitive3D::deserialize(InputStream *inInputStream)
{
    ...
    numBytes += inInputStream->read((unsigned char *)&mTransparent, 1);

    numBytes += inInputStream->read((unsigned char *)&mBackVisible, 1);
