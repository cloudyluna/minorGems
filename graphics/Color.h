...
class Color
{
  public:
     ...
     */
    static Color *makeColorFromHSV(float inHue, float inSaturation, float inValue, float inAlpha = 1,
     ...
     */
    static Color *linearSum(Color *inFirst, Color *inSecond, float inFirstWeight);
...
inline Color::Color(float red, float green, float blue, float alpha, char inBuildComposite)
{
    r = red;
    g = green;
    b = blue;
...
inline Color *Color::makeColorFromHSV(float inHue, float inSaturation, float inValue, float inAlpha,
                                      char inBuildComposite)
{
    ...
    else
    {
        ...
        float var_3 = inValue * (1 - inSaturation * (1 - (var_h - var_i)));
...
inline void Color::setValues(float red, float green, float blue, float alpha)
{
    r = red;
    g = green;
    b = blue;
...
{
    setValues(inOtherColor->r, inOtherColor->g, inOtherColor->b, inOtherColor->a);
}

inline char Color::equals(Color *inOtherColor)
{
    if (r == inOtherColor->r && g == inOtherColor->g && b == inOtherColor->b && a == inOtherColor->a)
    {
        return true;
    }
    else
...
inline Color *Color::linearSum(Color *inFirst, Color *inSecond, float inFirstWeight)
{
    ...
    return new Color(r, g, b, a, inFirst->mCompositeBuilt || inSecond->mCompositeBuilt);
}

inline unsigned long Color::rebuildComposite()
{
    composite = ((int)(lrint(b * 255))) | ((int)(lrint(g * 255))) << 8 | ((int)(lrint(r * 255))) << 16 |
                ((int)(lrint(a * 255))) << 24;
...
{
    return ((int)(lrint(b * 255 * weight))) | ((int)(lrint(g * 255 * weight))) << 8 |
           ((int)(lrint(r * 255 * weight))) << 16 | ((int)(lrint(a * 255))) << 24;
}
