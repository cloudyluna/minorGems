...
class Image : public Serializable
{
  ...
  public:
     ...
     */
    Image *getSubImage(int inStartX, int inStartY, int inWidth, int inHeight);
    ...
    void setSubImage(int inStartX, int inStartY, int inWidth, int inHeight, Image *inSourceImage);

    // centers this image in a new, larger image with a black or white
    // border
    Image *expandImage(int inExpandedWidth, int inExpandedHeight, char inWhiteBorder = false);
...
inline Image::Image(int inWidth, int inHeight, int inNumChannels, char inStartPixelsAtZero)
    : mWide(inWidth), mHigh(inHeight), mNumPixels(inWidth * inHeight), mNumChannels(inNumChannels),
      mChannels(new double *[inNumChannels]), mSelection(NULL)
{
...
inline void Image::paste(Image *inImage)
{
    ...
    {
        pasteChannel(inImage->getChannel(i), inImage->getChannelSelection(i), i);
    }
}
...
inline double *Image::copyChannel(int inChannel)
{
    ...
    memcpy(copiedChannel, mChannels[inChannel], sizeof(double) * mNumPixels);
...
// is very messy.
inline void Image::pasteChannel(double *inChannelData, double *inMask, int inChannel)
{
        ...
        if (inMask != NULL)
        {
            ...
            for (int i = 0; i < mNumPixels; i++)
            {
                thisChannel[i] = (thisChannel[i] * (1 - selection[i]) +
                                  (thisChannel[i] * (1 - inMask[i]) + inChannelData[i] * inMask[i]) * selection[i]);
            }
        }
        else
        {
            ...
            for (int i = 0; i < mNumPixels; i++)
            {
                thisChannel[i] = (thisChannel[i] * (1 - selection[i]) + inChannelData[i] * selection[i]);
            }
        }
    }
    else
    { // no selection in this image
        if (inMask != NULL)
        {
            // scale incoming data with its masks
            for (int i = 0; i < mNumPixels; i++)
            {
                thisChannel[i] = thisChannel[i] * (1 - inMask[i]) + inChannelData[i] * inMask[i];
            }
        }
        else
        {
...
inline Image *Image::getSubImage(int inStartX, int inStartY, int inWidth, int inHeight)
{
    ...
    for (int c = 0; c < mNumChannels; c++)
    {
        ...
        for (int y = inStartY; y < endY; y++)
        {
            ...
            // copy row
            memcpy(&(destChannel[destY * inWidth]), &(sourceChannel[y * mWide + inStartX]), sizeof(double) * inWidth);
...
inline void Image::setSubImage(int inStartX, int inStartY, int inWidth, int inHeight, Image *inSourceImage)
{
    ...
    for (int c = 0; c < mNumChannels; c++)
    {
        ...
        for (int y = inStartY; y < endY; y++)
        {
            ...
            // copy row
            memcpy(&(destChannel[y * mWide + inStartX]), &(sourceChannel[sourceY * sourceWidth]),
...
inline Image *Image::expandImage(int inExpandedWidth, int inExpandedHeight, char inWhiteBorder)
{
    Image *destImage = new Image(inExpandedWidth, inExpandedHeight, mNumChannels, !inWhiteBorder);
    ...
    for (int c = 0; c < mNumChannels; c++)
    {
        ...
        for (int y = 0; y < mHigh; y++)
        {
            ...
            // copy row
            memcpy(&(destChannel[destY * inExpandedWidth + xOffset]), &(sourceChannel[y * mWide]),
                   sizeof(double) * mWide);
...
{
    Image *fourChannelImage = new Image(getWidth(), getHeight(), 4, false);

    // copy first three
    for (int c = 0; c < 3; c++)
    {
        fourChannelImage->pasteChannel(getChannel(c), c);
    }
    ...
    int tI = fourChannelImage->getWidth() * (fourChannelImage->getHeight() - 1);
    ...
    int numPixels = fourChannelImage->getWidth() * fourChannelImage->getHeight();
...
static void rgbToHSL(double inR, double inG, double inB, double *outH, double *outS, double *outL)
{
...
static void hslToRGB(double inH, double inS, double inL, double *outR, double *outG, double *outB)
{
    double r, g, b;
