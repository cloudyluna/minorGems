...
class ImageColorConverter
{
  ...
  public:
     ...
     */
    static unsigned char *grayscaleToByteArray(Image *inImage, int inChannelNumber = 0);
     ...
     */
    static Image *byteArrayToGrayscale(unsigned char *inBytes, int inWidth, int inHeight);
  ...
  protected:
     ...
     *   Must be destroyed by caller.
     */
    static Image *coefficientConvert(Image *inImage, double inC00, double inC01, double inC02, double inC10,
                                     double inC11, double inC12, double inC20, double inC21, double inC22);
};
...
inline unsigned char *ImageColorConverter::grayscaleToByteArray(Image *inImage, int inChannelNumber)
{
...
inline Image *ImageColorConverter::byteArrayToGrayscale(unsigned char *inBytes, int inWidth, int inHeight)
{
...
inline Image *ImageColorConverter::RGBToYIQ(Image *inRGBImage)
{
    if (inRGBImage->getNumChannels() != 3)
    {
        printf("RGBtoYIQ requires a 3-channel image as input.\n");
        return NULL;
    }

    Image *yiqImage = coefficientConvert(inRGBImage, 0.299, 0.587, 0.114, 0.596, -0.274, -0.322, 0.212, -0.523, 0.311);
...
inline Image *ImageColorConverter::YIQToRGB(Image *inYIQImage)
{
    ...
    {
        printf("YIQtoRGB requires a 3-channel image as input.\n");
        return NULL;
    }

    Image *rgbImage = coefficientConvert(inYIQImage, 1.0, 0.956, 0.621, 1.0, -0.272, -0.647, 1.0, -1.105, 1.702);
    return rgbImage;
}
...
inline Image *ImageColorConverter::RGBToYCbCr(Image *inRGBImage)
{
    if (inRGBImage->getNumChannels() != 3)
    {
        printf("RGBtoYCbCr requires a 3-channel image as input.\n");
        return NULL;
    }
    ...
    */
    ...
    Image *ycbcrImage =
        coefficientConvert(inRGBImage, 0.2989, 0.5866, 0.1145, -0.1687, -0.3312, 0.5000, 0.5000, -0.4183, -0.0816);
...
inline Image *ImageColorConverter::YCbCrToRGB(Image *inYCbCrImage)
{
    ...
    {
        printf("YCbCrtoRGB requires a 3-channel image as input.\n");
        return NULL;
    }
    ...
    */
    ...
    Image *rgbImage = coefficientConvert(inYCbCrImage, 1.0, 0.0000, 1.4022, 1.0, -0.3456, -0.7145, 1.0, 1.7710, 0.0000);
...
inline Image *ImageColorConverter::coefficientConvert(Image *inImage, double inC00, double inC01, double inC02,
                                                      double inC10, double inC11, double inC12, double inC20,
                                                      double inC21, double inC22)
{
    ...
    for (int i = 0; i < numPixels; i++)
    {
        outChannel0[i] = inC00 * inChannel0[i] + inC01 * inChannel1[i] + inC02 * inChannel2[i];
        outChannel1[i] = inC10 * inChannel0[i] + inC11 * inChannel1[i] + inC12 * inChannel2[i];
        outChannel2[i] = inC20 * inChannel0[i] + inC21 * inChannel1[i] + inC22 * inChannel2[i];
    }
