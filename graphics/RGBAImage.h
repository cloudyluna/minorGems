...
class RGBAImage : public Image
{
  ...
  public:
    ...
    static Image *getImageFromBytes(unsigned char *inBytes, int inWidth, int inHeight, int inNumChannels);
...
inline RGBAImage::RGBAImage(int inWidth, int inHeight) : Image(inWidth, inHeight, 4)
{
}
...
inline RGBAImage::RGBAImage(Image *inImage)
    ...
    : Image(inImage->getWidth(), inImage->getHeight(), 4, (inImage->getNumChannels() < 4))
{
...
inline Image *RGBAImage::getImageFromBytes(unsigned char *inBytes, int inWidth, int inHeight, int inNumChannels)
{
    ...
    if (inNumChannels == 3)
    {
        ...
        for (int y = 0; y < inHeight; y++)
        {
            for (int x = 0; x < inWidth; x++)
            {
                ...
                channelOne[outputPixelIndex] = inBytes[byteIndex++] * 0.003921569;
                channelTwo[outputPixelIndex] = inBytes[byteIndex++] * 0.003921569;
                channelThree[outputPixelIndex] = inBytes[byteIndex++] * 0.003921569;
            }
            outputRow++;
        }
    }
    else if (inNumChannels == 4)
    {
        ...
        for (int y = 0; y < inHeight; y++)
        {
            for (int x = 0; x < inWidth; x++)
            {
                ...
                channelTwo[outputPixelIndex] = inBytes[byteIndex++] * 0.003921569;
                channelThree[outputPixelIndex] = inBytes[byteIndex++] * 0.003921569;
                channelFour[outputPixelIndex] = inBytes[byteIndex++] * 0.003921569;
            }
            outputRow++;
        }
    }
    else
    {
        ...
        for (int y = 0; y < inHeight; y++)
        {
            for (int x = 0; x < inWidth; x++)
            {
                ...
                for (int i = 0; i < inNumChannels; i++)
                {
                    channels[i][outputPixelIndex] = inBytes[byteIndex++] * 0.003921569;
                }
            }
            outputRow++;
        }
