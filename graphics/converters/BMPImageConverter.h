...
inline void BMPImageConverter::formatImage(Image *inImage, OutputStream *inStream)
{

    // make sure the image is in the right format
    if (inImage->getNumChannels() != 3)
    {
        printf("Only 3-channel images can be converted to the BMP format.\n");
        return;
    }
    ...
    for (int y = height - 1; y >= 0; y--)
    {
        for (int x = 0; x < width; x++)
        {
            ...
            raster[rasterIndex] = (unsigned char)(lrint(255 * blue[imageIndex]));
            raster[rasterIndex + 1] = (unsigned char)(lrint(255 * green[imageIndex]));
            raster[rasterIndex + 2] = (unsigned char)(lrint(255 * red[imageIndex]));
    ...
    else
    {
        ...
        for (int y = height - 1; y >= 0; y--)
        {
            for (int x = 0; x < width; x++)
            {
                ...
                blue[imageIndex] = (double)(raster[rasterIndex]) / 255.0;
                green[imageIndex] = (double)(raster[rasterIndex + 1]) / 255.0;
                red[imageIndex] = (double)(raster[rasterIndex + 2]) / 255.0;
