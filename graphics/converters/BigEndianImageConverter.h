...
class BigEndianImageConverter : public ImageConverter
{
  ...
  protected:
     ...
     */
    void writeBigEndianLong(long inLong, OutputStream *inStream);
     ...
     */
    void writeBigEndianShort(short inShort, OutputStream *inStream);
...
inline void BigEndianImageConverter::writeBigEndianLong(long inLong, OutputStream *inStream)
{
...
inline long BigEndianImageConverter::readBigEndianLong(InputStream *inStream)
{
    ...
    long outLong = (buffer[0] << 24) | (buffer[1] << 16) | (buffer[2] << 8) | buffer[3];

    return outLong;
}

inline void BigEndianImageConverter::writeBigEndianShort(short inShort, OutputStream *inStream)
{
...
inline short BigEndianImageConverter::readBigEndianShort(InputStream *inStream)
{
    ...
    long outShort = (buffer[0] << 8) | buffer[1];
