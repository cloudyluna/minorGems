...
PNGImageConverter::PNGImageConverter(int inCompressionLevel) : mCompressionLevel(inCompressionLevel)
{
...
unsigned long PNGImageConverter::updateCRC(unsigned long inCRC, unsigned char *inData, int inLength)
{
    ...
    {
        c = mCRCTable[(c ^ inData[n]) & 0xff] ^ (c >> 8);
    }
    return c;
}
...
void PNGImageConverter::writeChunk(const char inChunkType[4], unsigned char *inData, unsigned long inNumBytes,
                                   OutputStream *inStream)
{
    ...
    // start the crc
    unsigned long crc = updateCRC(mStartCRC, (unsigned char *)inChunkType, 4);
...
void libpngWriteCallback(png_structp png_ptr, png_bytep data, png_size_t length)
{
...
void PNGImageConverter::formatImage(Image *inImage, OutputStream *inStream)
{
    ...
    png_set_write_fn(png_ptr, (void *)inStream, libpngWriteCallback, libpngFlushCallback);
    ...
    png_set_IHDR(png_ptr, info_ptr, w, h, 8, PNG_COLOR_TYPE_RGB_ALPHA, PNG_INTERLACE_NONE, PNG_COMPRESSION_TYPE_BASE,
    ...
    */
    ...
    for (int y = 0; y < h; y++)
    {
        ...
        for (int x = 0; x < w; x++)
        {
            ...
            for (i = 0; i < 3; i++)
            {
                rawScanlineBytes[index] = (unsigned char)(lrint(255 * channels[i][pixelNumber]));
...
void libpngReadCallback(png_structp png_ptr, png_bytep data, png_size_t length)
{
...
Image *PNGImageConverter::deformatImage(InputStream *inStream)
{
    ...
    png_set_read_fn(png_ptr, (void *)inStream, libpngReadCallback);
