...
class PNGImageConverter : public BigEndianImageConverter
{
  ...
  protected:
     ...
     *   caller.
     */
    void writeChunk(const char inChunkType[4], unsigned char *inData, unsigned long inNumBytes, OutputStream *inStream);
     ...
     * @return the new CRC.
     */
    unsigned long updateCRC(unsigned long inCRC, unsigned char *inData, int inLength);
};
