...
class TGAImageConverter : public LittleEndianImageConverter
{
  ...
  public:
    ...
    virtual void formatImage(Image *inImage, OutputStream *inStream);
...
inline void TGAImageConverter::formatImage(Image *inImage, OutputStream *inStream)
{
    ...
    if (numChannels == 3)
    {
        ...
        {
            raster[rasterIndex] = (unsigned char)(lrint(255 * blue[i]));
            raster[rasterIndex + 1] = (unsigned char)(lrint(255 * green[i]));
            raster[rasterIndex + 2] = (unsigned char)(lrint(255 * red[i]));
    ...
    { // numChannels == 4
        ...
        for (int i = 0; i < numPixels; i++)
        {
            ...
            raster[rasterIndex + 1] = (unsigned char)(lrint(255 * green[i]));
            raster[rasterIndex + 2] = (unsigned char)(lrint(255 * red[i]));
            raster[rasterIndex + 3] = (unsigned char)(lrint(255 * alpha[i]));
...
inline RawRGBAImage *TGAImageConverter::deformatImageRaw(InputStream *inStream)
{
    ...
    if (byteBuffer[0] != 2)
    {
        printf("Only TGA files containing unmapped RGB images can be read.\n");
        delete[] byteBuffer;
        return NULL;
    }
    ...
    {
        // We skip the image identification field
        unsigned char *identificationField = new unsigned char[identificationFieldSize];
    ...
    if (!originAtTop)
    {
        ...
        // flip it
        unsigned char *flippedRaster = new unsigned char[numPixels * numChannels];

        int lineBytes = width * numChannels;

        for (int y = 0; y < height; y++)
        {
            memcpy(&(flippedRaster[lineBytes * y]), &(raster[lineBytes * (height - y - 1)]), lineBytes);
        }
