...
inline BoxBlurFilter::BoxBlurFilter(int inRadius) : mRadius(inRadius)
{
}
...
inline void BoxBlurFilter::apply(double *inChannel, int inWidth, int inHeight)
{
        ...
        for (int x = 0; x < inWidth; x++)
        {
            ...
            inChannel[y * inWidth + x] =
                boxValueMultiplier * (
                                         ...
                                         - yOutsideFactor * accumTotals[boxYStart * inWidth + boxXEnd] -
                                         xOutsideFactor * accumTotals[boxYEnd * inWidth + boxXStart]
                                         // add back in region that was subtracted twice, if any
                                         + outsideOverlapFactor * accumTotals[boxYStart * inWidth + boxXStart]);
        }
    }
