...
inline void FastBlurFilter::apply(double *inChannel, int inWidth, int inHeight)
{
    ...
    for (int y = 1; y < inHeight - 1; y++)
    {
        ...
        for (int i = 0; i < numPixelsInRow; i++)
        {
            ...
            inChannel[pixelIndex] =
                (sourcePointer[0] + sourcePointer[cornerOffsetA] + sourcePointer[cornerOffsetB] +
                 sourcePointer[cornerOffsetC] + sourcePointer[cornerOffsetD] + sourcePointer[sideOffsetA] +
                 sourcePointer[sideOffsetB] + sourcePointer[sideOffsetC] + sourcePointer[sideOffsetD]) *
                boxMultiplier;
