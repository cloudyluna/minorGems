...
inline MedianFilter::MedianFilter(int inRadius) : mRadius(inRadius)
{
}
...
inline void MedianFilter::apply(double *inChannel, int inWidth, int inHeight)
{
    ...
    for (int y = 0; y < inHeight; y++)
    {
        ...
        for (int x = 0; x < inWidth; x++)
        {
            ...
            for (int boxY = startBoxY; boxY <= endBoxY; boxY++)
            {
                ...
                {
                    // buffer[boxSizeX*(boxY-startBoxY)+(boxX-startBoxX)] = (int)(1000.0 * inChannel[yBoxIndexContrib +
                    // boxX]);
