...
inline SeamlessFilter::SeamlessFilter(double inBlendCurveExponent) : mBlendingCurveExponent(inBlendCurveExponent)
{
}

inline void SeamlessFilter::apply(double *inChannel, int inWidth, int inHeight)
{
    ...
    for (int x = 0; x < inWidth; x++)
    {
        ...
        for (int y = 0; y < inHeight; y++)
        {
            ...
            channelCopy[y * inWidth + x] =
                (1 - oXFraction) * inChannel[y * inWidth + x] + oXFraction * inChannel[y * inWidth + oX];
        }
    }
    ...
    for (int y = 0; y < inHeight; y++)
    {
        ...
        for (int x = 0; x < inWidth; x++)
        {
            ...
            // add in y overlay
            channelCopy[y * inWidth + x] =
                (1 - oYFraction) * inChannel[y * inWidth + x] + oYFraction * inChannel[oY * inWidth + x];
        }
    }
