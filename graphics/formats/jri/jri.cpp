...
rgbaColor *extractJRI(unsigned char *inData, int inNumBytes, int *outWidth, int *outHeight)
{

    int version, w, h, numColors;

    int numRead = sscanf((char *)inData, "%d %d %d %d", &version, &w, &h, &numColors);
...
static void outputNonRunBytes(SimpleVector<unsigned char> *dataVector, SimpleVector<unsigned char> *nonRunBytes)
{
    ...
    if (numNonRun > 0)
    {
        ...
        for (int b = 0; b < numNonRun; b++)
        {
            ...
            dataVector->push_back(*(nonRunBytes->getElement(b)));
        }
        nonRunBytes->deleteAll();
    }
}

unsigned char *generateJRI(rgbaColor *inRGBA, int inWidth, int inHeight, int *outNumBytes)
{
    ...
    char *header = autoSprintf("%d\n%d %d\n%d\n", JRI_VERSION, inWidth, inHeight, colors.size());
