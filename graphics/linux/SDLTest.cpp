...
int main()
{
    ...
    for (int y = 0; y < 10; y++)
    {
        for (int x = 0; x < 10; x++)
        {
            mouseMap1->imageMap[mouseMap1->yOffset[y] + x] = mouseColor1.composite;
        }
    }
    ...
    for (int y = 0; y < 10; y++)
    {
        for (int x = 0; x < 10; x++)
        {
            mouseMap2->imageMap[mouseMap2->yOffset[y] + x] = mouseColor2.composite;
        }
    }
    ...
    for (int y = 0; y < 10; y++)
    {
        for (int x = 0; x < 10; x++)
        {
            mouseMap3->imageMap[mouseMap3->yOffset[y] + x] = mouseColor3.composite;
        }
    }
    ...
    for (i = 0; i < numIcons; i++)
    {
        ...
        Color randColor((float)rand() / RAND_MAX, (float)rand() / RAND_MAX, (float)rand() / RAND_MAX, 1.0);
        for (int y = 0; y < 10; y++)
        {
            for (int x = 0; x < 10; x++)
            {
                maps[i]->imageMap[maps[i]->yOffset[y] + x] = randColor.composite;
            }
        }
    }
    int mouseX = 0;
    ...
    for (currentStep = 0; currentStep < numSteps; currentStep++)
    {
        ...
        for (i = 0; i < numIcons; i++)
        {
            buffer->eraseIconMap(maps[i], (int)(xPos[i] - 5), (int)(yPos[i] - 5), c);
            ...
            buffer->drawIconMap(maps[i], (int)(xPos[i] - 5), (int)(yPos[i] - 5));
        }
