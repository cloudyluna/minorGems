...
ScreenGraphics::ScreenGraphics(int inWidth, int inHeight) : mWidth(inWidth), mHeight(inHeight)
{
    ...
    SDL_Surface *screen = SDL_SetVideoMode(mWidth, mHeight, 32, SDL_SWSURFACE);
...
char ScreenGraphics::isResolutionAvailable(int inWidth, int inHeight)
{
    ...
    char videoModeOk = (char)SDL_VideoModeOK(inWidth, inHeight, 32, SDL_SWSURFACE);
...
{
    if (inOutBuffer->getHeight() != mHeight || inOutBuffer->getWidth() != mWidth)
    {
    ...
    memcpy((void *)buffer, (void *)(inOutBuffer->getBuffer()), mWidth * mHeight * 4);
...
void ScreenGraphics::flipDirtyRectangle(int inTopLeftX, int inTopLeftY, int inWidth, int inHeight)
{
