...
class PrimitiveGL
{
  ...
  protected:
    ...
    void drawNoMultitexture(Transform3D *inTransform, LightingGL *inLighting);
};

inline PrimitiveGL::PrimitiveGL(Primitive3D *inPrimitive) : mPrimitive(inPrimitive)
{
    ...
    for (int i = 0; i < numTextures; i++)
    {
        ...
        mTextureGL->setTextureData(i, rgba, mPrimitive->mTexture[i]->getWidth(), mPrimitive->mTexture[i]->getHeight());
...
inline void PrimitiveGL::draw(Transform3D *inTransform, LightingGL *inLighting)
{
    ...
    for (int y = 0; y < mPrimitive->mHigh - 1; y++)
    {
        ...
        inLighting->getLighting(worldVertices[index], worldNormals[index], lightColor);
        glColor4f(lightColor->r, lightColor->g, lightColor->b, 1.0);

        // pass in each layer's anchor points
        for (t = 0; t < numTextureLayers; t++)
        {
            glMultiTexCoord2fARB(TextureGL::sMultiTextureEnum[t], mPrimitive->mAnchorX[t][index],
                                 mPrimitive->mAnchorY[t][index]);
        }
        glVertex3d(worldVertices[index]->mX, worldVertices[index]->mY, worldVertices[index]->mZ);
...
inline void PrimitiveGL::drawNoMultitexture(Transform3D *inTransform, LightingGL *inLighting)
{
    ...
    for (int y = 0; y < mPrimitive->mHigh - 1; y++)
    {
        ...
        glTexCoord2d(mPrimitive->mAnchorX[0][index], mPrimitive->mAnchorY[0][index]);
        glVertex3d(worldVertices[index]->mX, worldVertices[index]->mY, worldVertices[index]->mZ);
