...
class ScreenGL
{
  ...
  public:
     ...
     *   Must be destroyed by caller.
     */
    ScreenGL(int inWide, int inHigh, char inFullScreen, char inDoNotChangeNativeResolution, unsigned int inMaxFrameRate,
             char inRecordEvents, const char *inCustomRecordedGameData, const char *inHashSalt,
             const char *inWindowName, KeyboardHandlerGL *inKeyHandler = NULL, MouseHandlerGL *inMouseHandler = NULL,
    ...
    void setKeyMapping(unsigned char inFromKey, unsigned char inToKey);
    ...
    //        (NULL body)
    void registerWebEvent(int inHandle, int inType, const char *inBodyString = NULL,
    ...
    char *getWebEventResultBody(int inHandle, int *outSize = NULL);
    ...
    void registerSocketEvent(int inHandle, int inType, int inNumBodyBytes, unsigned char *inBodyBytes = NULL);
    ...
    //
    //
    void getSocketEventTypeAndSize(int inHandle, int *outType, int *outNumBodyBytes);
     ...
     * Must not be called after calling start().
     */
    void addKeyboardHandler(KeyboardHandlerGL *inHandler, char inFirstHandler = false);
...
inline void ScreenGL::addKeyboardHandler(KeyboardHandlerGL *inListener, char inFirstHandler)
{
    if (!inFirstHandler)
    {
...
inline char ScreenGL::isKeyboardHandlerFocused()
{
    ...
    {
        KeyboardHandlerGL *handler = *(mKeyboardHandlerVector->getElement(h));
