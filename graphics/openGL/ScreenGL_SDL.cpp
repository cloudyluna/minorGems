...
ScreenGL::ScreenGL(int inWide, int inHigh, char inFullScreen, char inDoNotChangeNativeResolution,
                   unsigned int inMaxFrameRate, char inRecordEvents, const char *inCustomRecordedGameData,
                   const char *inHashSalt, const char *inWindowName, KeyboardHandlerGL *inKeyHandler,
                   MouseHandlerGL *inMouseHandler, SceneHandlerGL *inSceneHandler)
    : mWide(inWide), mHigh(inHigh), mForceAspectRatio(false), mForceSpecifiedDimensions(false),
      mDoNotChangeNativeResolution(inDoNotChangeNativeResolution), mImageWide(inWide), mImageHigh(inHigh),
      mFullScreen(inFullScreen), mMaxFrameRate(inMaxFrameRate), mUseFrameSleep(true), mFullFrameRate(inMaxFrameRate),
      m2DMode(false), mViewPosition(new Vector3D(0, 0, 0)), mViewOrientation(new Angle3D(0, 0, 0)),
      ...
      mRedrawListenerVector(new SimpleVector<RedrawListenerGL *>())
{
    ...
    if (numChildren > 0)
    {
        ...
        // skip hidden files
        int i = 0;
        while (partialFileName != NULL && partialFileName[i] == '.')
        {
        ...
        if (fullFileName != NULL)
        {
            ...
            else
            {
                ...
                AppLog::getLog()->logPrintf(Log::INFO_LEVEL, "Playing back game from file %s", fullFileName);
                ...
                int numScanned =
                    fscanf(mEventFile, "%u seed, %u fps, %dx%d, fullScreen=%d, %s %40s\n", &readRandSeed,
                           &readMaxFrameRate, &readWide, &readHigh, &fullScreenFlag, readCustomGameData, hashString);

                if (numScanned == 7)
                {

                    char *stringToHash = autoSprintf("%s%s", readCustomGameData, mHashSalt);
                    ...
                    if (difference == 0)
                    {
                        ...
                        mCustomRecordedGameData = stringDuplicate(readCustomGameData);
                    }
                    else
                    {
                        AppLog::error("Hash check failed for custom data in playback file");
                    }
                }
                else
                {
                    AppLog::error("Failed to parse playback header data");
                }
...
void ScreenGL::startRecordingOrPlayback()
{
    if (mRecordingEvents)
    {
        ...
        char *fileName = autoSprintf("recordedGame%06d.txt", fileNumber);
        ...
        {
            AppLog::getLog()->logPrintf(Log::INFO_LEVEL, "Recording game into file %s", fullFileName);
            ...
            char *stringToHash = autoSprintf("%s%s", mCustomRecordedGameData, mHashSalt);

            char *correctHash = computeSHA1Digest(stringToHash);

            delete[] stringToHash;

            fprintf(mEventFile, "%u seed, %u fps, %dx%d, fullScreen=%d, %s %s\n", mRandSeed, mMaxFrameRate, mWide,
                    mHigh, fullScreenFlag, mCustomRecordedGameData, correctHash);
        ...
        int keepNumber = SettingsManager::getIntSetting("keepPastRecordings", -1);
        if (keepNumber >= 0)
        {

            AppLog::getLog()->logPrintf(Log::INFO_LEVEL, "Only keeping %d past recordings", keepNumber);
            ...
            AppLog::getLog()->logPrintf(Log::INFO_LEVEL, "Removed %d stale recordings", numRemoved);
        }
    }
...
void ScreenGL::setupSurface()
{
    ...
    AppLog::getLog()->logPrintf(Log::INFO_LEVEL, "Current screen configuration is %dx%d with aspect ratio %.2f",
    ...
    if (modes == (SDL_Rect **)-1)
    {
        ...
        if (mFullScreen && mDoNotChangeNativeResolution)
        {
            ...
            int borderlessHeightAdjust = SettingsManager::getIntSetting("borderlessHeightAdjust", 0);
            ...
            if (borderless && borderlessHeightAdjust != 0)
            {
                AppLog::getLog()->logPrintf(Log::INFO_LEVEL,
                                            ...
                                            borderlessHeightAdjust, mHigh, mHigh + borderlessHeightAdjust);
    ...
    else if (mForceSpecifiedDimensions && mFullScreen)
    {
        ...
        if (!match)
        {
            AppLog::getLog()->logPrintf(Log::WARNING_LEVEL,
    ...
    else
    {
        ...
        AppLog::getLog()->logPrintf(Log::INFO_LEVEL, "Checking if requested video mode (%dx%d) is available", mWide,
                                    mHigh);
        ...
        {
            AppLog::getLog()->logPrintf(Log::DETAIL_LEVEL, "   %d x %d\n", modes[i]->w, modes[i]->h);

            int thisAspectRatio = computeAspectRatio(modes[i]->w, modes[i]->h);
        ...
        if (!match)
        {
            ...
            for (int i = 0; modes[i]; ++i)
            {
                ...
                if (modes[i]->w >= mWide && modes[i]->h >= mHigh)
                {

                    int distance = (int)(fabs(modes[i]->w - mWide) + fabs(modes[i]->h - mHigh));

                    int thisAspectRatio = computeAspectRatio(modes[i]->w, modes[i]->h);

                    if ((mForceAspectRatio || thisAspectRatio == currentAspectRatio) && distance < bestDistance)
                    {
            ...
            if (bestIndex != -1)
            {
                ...
                if (mForceAspectRatio)
                {
                    AppLog::getLog()->logPrintf(Log::INFO_LEVEL,
                                                ...
                                                "%d x %d\n",
                                                modes[bestIndex]->w, modes[bestIndex]->h);
                }
                else
                {
                    AppLog::getLog()->logPrintf(Log::INFO_LEVEL,
            ...
            else
            {
                ...
                if (!mForceAspectRatio)
                {
                    ...
                    AppLog::warning("Warning:  No match for current aspect ratio");
                ...
                {
                    AppLog::warning("Warning:  No sufficiently sized resolution found");
                    AppLog::info("Considering closest-match smaller resolution");
...
void ScreenGL::registerWebEvent(int inHandle, int inType, const char *inBodyString, int inBodyLength)
{
    ...
    if (inType == 2)
    {
        if (inBodyLength == -1)
        {
            ...
            eventString = autoSprintf("wb %u %d %u %s", inHandle, inType, strlen(inBodyString), inBodyString);
        }
        else
        {

            char *bodyHex = hexEncode((unsigned char *)inBodyString, inBodyLength);

            eventString = autoSprintf("wx %u %d %u %s", inHandle, inType, strlen(bodyHex), bodyHex);
...
void ScreenGL::registerSocketEvent(int inHandle, int inType, int inNumBodyBytes, unsigned char *inBodyBytes)
{
    ...
    if (inType == 2 && inNumBodyBytes != 0)
    {
        ...
        eventString = autoSprintf("xs %u %d %u %s", inHandle, inType, inNumBodyBytes, bodyHex);
        delete[] bodyHex;
    }
    else
    {
        eventString = autoSprintf("xs %u %d %d", inHandle, inType, inNumBodyBytes);
    }

    mEventBatch.push_back(eventString);
}

void ScreenGL::getSocketEventTypeAndSize(int inHandle, int *outType, int *outNumBodyBytes)
{
    for (int i = 0; i < mPendingSocketEvents.size(); i++)
    {
        SocketEvent *e = mPendingSocketEvents.getElement(i);
...
void ScreenGL::writeEventBatchToFile(SimpleVector<char *> *inBatch)
{
    ...
    if (mEventFile != NULL)
    {
        if (numInBatch > 0)
        {
            ...
            int numWritten = fwrite(eventString, 1, eventStringLength, mEventFile);
    ...
    for (int i = 0; i < batchSize; i++)
    {
        ...
        case 'm':
            ...
            case 'b': {
                ...
                fscanf(mEventFile, "%d %d %d %d", &button, &state, &x, &y);
        ...
        case 'w': {
            ...
            if (e.type == 2)
            {
                ...
                if (code[1] == 'b')
                {
                    ...
                    unsigned int numRead = fread(e.bodyText, 1, length, mEventFile);

                    e.bodyText[length] = '\0';

                    if (numRead != length)
                    {
                        AppLog::error("Failed to read web event body text from "
                                      "playback file");
                        delete[] e.bodyText;
                        e.bodyText = NULL;
                        e.bodyLength = 0;
                ...
                else if (code[1] == 'x')
                {
                    ...
                    unsigned int numRead = fread(bodyHex, 1, length, mEventFile);

                    bodyHex[length] = '\0';

                    if (numRead != length)
                    {
                        AppLog::error("Failed to read web event body hex from "
                                      "playback file");
                        e.bodyText = NULL;
                        e.bodyLength = 0;
                    }
                    ...
                    {
                        unsigned char *bodyDecoded = hexDecode(bodyHex);
        ...
        case 'x': {
            ...
            SocketEvent e;
            fscanf(mEventFile, "%d %d %d", &(e.handle), &(e.type), &(e.numBodyBytes));
            ...
            if (e.type == 2 && e.numBodyBytes != 0)
            {
                ...
                unsigned int numRead = fread(e.bodyBytesHex, 1, hexLength, mEventFile);

                e.bodyBytesHex[hexLength] = '\0';

                if (numRead != hexLength)
                {
                    AppLog::error("Failed to read socket event body hex from "
                                  "playback file");
                    delete[] e.bodyBytesHex;
                    e.bodyBytesHex = NULL;
                }
        ...
        break;
        default:
            AppLog::getLog()->logPrintf(Log::ERROR_LEVEL, "Unknown code '%s' in playback file\n", code);
        }
    }
...
char ScreenGL::isMinimized()
{
    ...
    if (isMin && mRecordingEvents && mRecordingOrPlaybackStarted)
    {
...
void ScreenGL::start()
{
    ...
    while (true)
    {
        ...
        while (!(mPlaybackEvents && mRecordingOrPlaybackStarted) && SDL_PollEvent(&event))
        {
            ...
            if (mStartedFullScreen && event.type == SDL_KEYDOWN && event.key.keysym.sym == SDLK_RETURN &&
            ...
            else if (!mFullScreen && !mMinimized && event.type == SDL_KEYDOWN && event.key.keysym.sym == SDLK_TAB &&
                     ((mods & KMOD_META) || (mods & KMOD_ALT)))
            {
                ...
                if (mRecordingEvents && mRecordingOrPlaybackStarted)
                {
                    ...
                    char *eventString = autoSprintf("kd %d %d %d", 9, mouseX, mouseY);
            ...
            else if (mMinimized && !mWasFullScreenBeforeMinimize && event.type == SDL_ACTIVEEVENT &&
                     event.active.gain && event.active.state == SDL_APPACTIVE)
            {
            ...
            else if (mMinimized && mWasFullScreenBeforeMinimize && event.type == SDL_ACTIVEEVENT && event.active.gain &&
            ...
            else
            {
                ...
                case SDL_MOUSEMOTION:
                    if (event.motion.state & SDL_BUTTON(1) || event.motion.state & SDL_BUTTON(2) ||
                    ...
                    {
                        callbackPassiveMotion(event.motion.x, event.motion.y);
                    }
                    break;
        ...
        if (mPlaybackEvents && mRecordingOrPlaybackStarted && mEventFile != NULL)
        {
            ...
            while (SDL_PollEvent(&event))
            {
                ...
                if (event.type == SDL_KEYDOWN && ((event.key.keysym.sym == SDLK_q &&
                                                   ((mods & KMOD_META) || (mods & KMOD_ALT) || (mods & KMOD_CTRL))) ||
                ...
                else
                {
                    ...
                    case SDL_KEYDOWN: {
                        ...
                        {
                            mShouldShowPlaybackDisplay = !mShouldShowPlaybackDisplay;
                        }
            ...
            if (!mPlaybackEvents)
            {
                ...
                int mouseX, mouseY;
                SDL_GetMouseState(&mouseX, &mouseY);
                callbackMouse(SDL_BUTTON_LEFT, SDL_RELEASED, mouseX, mouseY);

                callbackMouse(SDL_BUTTON_MIDDLE, SDL_RELEASED, mouseX, mouseY);

                callbackMouse(SDL_BUTTON_RIGHT, SDL_RELEASED, mouseX, mouseY);

                callbackMouse(SDL_BUTTON_WHEELUP, SDL_RELEASED, mouseX, mouseY);

                callbackMouse(SDL_BUTTON_WHEELDOWN, SDL_RELEASED, mouseX, mouseY);
        ...
        if (mUseFrameSleep)
        {
            ...
            {
                int timeToSleep = minFrameTime - (frameTime + oversleepMSec);
                ...
                int actualSleepTime = Time::getMillisecondsSince(sleepStartSec, sleepStartMSec);
...
void ScreenGL::setKeyMapping(unsigned char inFromKey, unsigned char inToKey)
{
    keyMap[inFromKey] = inToKey;

    AppLog::getLog()->logPrintf(Log::INFO_LEVEL, "Mapping key '%c' to '%c'", inFromKey, inToKey);
}
...
void ScreenGL::applyViewTransform()
{
    ...
    double angle = atan(tan(M_PI / 4) + aspectDifference / zSlice);
    ...
    gluLookAt(mViewPosition->mX, mViewPosition->mY, mViewPosition->mZ, viewDirection->mX, viewDirection->mY,
              viewDirection->mZ, upDirection->mX, upDirection->mY, upDirection->mZ);
...
void callbackResize(int inW, int inH)
{
    ...
    glViewport(excessW / 2, excessH / 2, bigDimension, bigDimension);
}
...
void callbackKeyboardUp(unsigned char inKey, int inX, int inY)
{
    ...
    if (currentScreenGL->mRecordingEvents && currentScreenGL->mRecordingOrPlaybackStarted)
    {
        ...
        char *eventString = autoSprintf("ku %d %d %d", keyToRecord, inX, inY);
...
void callbackMotion(int inX, int inY)
{
    ...
    {
        MouseHandlerGL *handler = *(currentScreenGL->mMouseHandlerVector->getElement(h));
        handler->mHandlerFlagged = true;
    }
...
void callbackMouse(int inButton, int inState, int inX, int inY)
{
    ...
    if (currentScreenGL->mRecordingEvents && currentScreenGL->mRecordingOrPlaybackStarted)
    {
        ...
        char *eventString = autoSprintf("mb %d %d %d %d", inButton, stateEncoding, inX, inY);
