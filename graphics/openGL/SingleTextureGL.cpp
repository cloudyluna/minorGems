...
void SingleTextureGL::reloadFromBackup()
{
    ...
    {
        setTextureData(mBackupBytes, mAlphaOnly, mWidthBackup, mHeightBackup,
                       // backup already has edges expanded
                       false);
    }
}

SingleTextureGL::SingleTextureGL(Image *inImage, char inRepeat, char inMipMap)
    : mRepeat(inRepeat), mMipMap(inMipMap), mAlphaOnly(false), mBackupBytes(NULL)
{
...
SingleTextureGL::SingleTextureGL(unsigned char *inRGBA, unsigned int inWidth, unsigned int inHeight, char inRepeat,
                                 char inMipMap)
    : mRepeat(inRepeat), mMipMap(inMipMap), mAlphaOnly(false), mBackupBytes(NULL)
{
...
SingleTextureGL::SingleTextureGL(char inAlphaOnly, unsigned char *inA, unsigned int inWidth, unsigned int inHeight,
                                 char inRepeat, char inMipMap)
    : mRepeat(inRepeat), mMipMap(inMipMap), mAlphaOnly(true), mBackupBytes(NULL)
{
...
void SingleTextureGL::setTextureData(Image *inImage)
{
    ...
    setTextureData(textureData, false, inImage->getWidth(), inImage->getHeight(), true);

    delete[] textureData;
}

void SingleTextureGL::replaceBackupData(unsigned char *inBytes, char inAlphaOnly, unsigned int inWidth,
                                        unsigned int inHeight)
{
    if (inBytes != mBackupBytes)
    {
        ...
        mBackupBytes = new unsigned char[inWidth * inHeight * numBytesPerPixel];
...
void SingleTextureGL::setTextureData(unsigned char *inBytes, char inAlphaOnly, unsigned int inWidth,
                                     unsigned int inHeight, char inExpandEdge)
{
    ...
    if (inExpandEdge && !inAlphaOnly)
    {
        ...
        if (minY < maxY && minX < maxX && minY > 0 && maxY < inHeight - 1 && minX > 0 && maxX < inWidth - 1)
        {
            ...
            if (solidPresent)
            {
                memcpy(&(inBytes[rowDestStart]), &(inBytes[rowStart]), inWidth * 4);
            }
    ...
    if (mMipMap)
    {
        ...
        glTexImage2D(GL_TEXTURE_2D, 0, internalTexFormat, inWidth, inHeight, 0, texDataFormat, GL_UNSIGNED_BYTE,
                     inBytes);
#else
        gluBuild2DMipmaps(GL_TEXTURE_2D, internalTexFormat, inWidth, inHeight, texDataFormat, GL_UNSIGNED_BYTE,
                          inBytes);
#endif
    }
    else
...
void SingleTextureGL::setWrapping(char inHorizontal, char inVertical)
{
...
void SingleTextureGL::replaceTextureData(unsigned char *inBytes, char inAlphaOnly, unsigned int inWidth,
                                         unsigned int inHeight)
{
    ...
    glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, // offset
                    inWidth, inHeight, texDataFormat, GL_UNSIGNED_BYTE, inBytes);
