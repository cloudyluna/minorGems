 ...
 * @author Jason Rohrer
 */
class TestHandlerGL : public MouseHandlerGL, public KeyboardHandlerGL, public SceneHandlerGL
{
...
TestHandlerGL::TestHandlerGL(RandomSource *inRandSource, int inNumTriangles)
    : mRandSource(inRandSource), mNumTriangles(inNumTriangles), mTriangles(new Vector3D **[inNumTriangles]),
      mColors(new Color **[inNumTriangles])
{

    mLightingA = new DirectionLightingGL(new Color(1.0, 1.0, 1.0), new Vector3D(0, 0, 1));

    mLightingB = new DirectionLightingGL(new Color(0, 0, 1.0), new Vector3D(1, 1, 0));
    ...
    for (int i = 0; i < mNumTriangles; i++)
    {
        ...
        {
            mTriangles[i][j] =
                new Vector3D(mRandSource->getRandomDouble() * 10, mRandSource->getRandomDouble() * 10 - 5,
                             mRandSource->getRandomDouble() * 10);
            mColors[i][j] = new Color(mRandSource->getRandomFloat(), mRandSource->getRandomFloat(),
                                      mRandSource->getRandomFloat(), mRandSource->getRandomFloat());
        }
    }
}
...
void TestHandlerGL::setupPrimitives()
{
    ...
    genFractalNoise2d(noiseImage->getChannel(0), textSize, textSize, fPower, true, mRandSource);
    genFractalNoise2d(noiseImage->getChannel(1), textSize, textSize, fPower, true, mRandSource);
    genFractalNoise2d(noiseImage->getChannel(2), textSize, textSize, fPower, true, mRandSource);

    Image *selection = new Image(textSize, textSize, 1);
    genFractalNoise2d(selection->getChannel(0), textSize, textSize, fPower, true, mRandSource);
    ...
    */
    ...
    Primitive3D *simplePrimitive3D = new Primitive3D(2, 2, corners, 2, imageArray, anchorArrayX, anchorArrayY);
    ...
    Primitive3D *lathePrimitive3D = new LathePrimitive3D(4, latheCurve, 15, 2 * M_PI, noiseImage);
    ...
    LandscapePrimitive3D *landscapePrimitive3D = new LandscapePrimitive3D(landSize, landSize, heights, noiseImage);
