...
class MouseCursorRegionGL : public GUIComponentGL
{
  ...
  public:
     ...
     *   Will be destroyed when this class is destroyed.
     */
    MouseCursorRegionGL(double inAnchorX, double inAnchorY, double inWidth, double inHeight, double inCursorFraction,
                        Color *inUnpressedColor, Color *inPressedColor);
...
inline MouseCursorRegionGL::MouseCursorRegionGL(double inAnchorX, double inAnchorY, double inWidth, double inHeight,
                                                double inCursorFraction, Color *inUnpressedColor, Color *inPressedColor)
    : GUIComponentGL(inAnchorX, inAnchorY, inWidth, inHeight), mCursorFraction(inCursorFraction),
      mUnpressedColor(inUnpressedColor), mPressedColor(inPressedColor), mMouseLocationX(-1), mMouseLocationY(-1)
{
...
inline void MouseCursorRegionGL::fireRedraw()
{
    ...
    {
        // set our color
        glColor4f(mCurrentColor->r, mCurrentColor->g, mCurrentColor->b, 1.0f);
        ...
        glBegin(GL_LINES);
        {
            glVertex2d(mMouseLocationX - 0.5 * cursorLineLengthX, mMouseLocationY);
            glVertex2d(mMouseLocationX + 0.5 * cursorLineLengthX, mMouseLocationY);
        }
        glEnd();
        ...
        glBegin(GL_LINES);
        {
            glVertex2d(mMouseLocationX, mMouseLocationY - 0.5 * cursorLineLengthY);
            glVertex2d(mMouseLocationX, mMouseLocationY + 0.5 * cursorLineLengthY);
        }
        glEnd();
    }
