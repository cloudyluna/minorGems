...
class SliderGL : public GUIComponentGL, public ActionListenerList
{
  ...
  public:
     ...
     */
    SliderGL(double inAnchorX, double inAnchorY, double inWidth, double inHeight, Image *inIconImage,
             double inIconWidthFraction, Color *inBarStartColor, Color *inBarEndColor, Color *inThumbColor,
             Color *inBorderColor, double inBorderWidth, double inThumbWidth, double inMinThumbIncrement);
...
inline SliderGL::SliderGL(double inAnchorX, double inAnchorY, double inWidth, double inHeight, Image *inIconImage,
                          double inIconWidthFraction, Color *inBarStartColor, Color *inBarEndColor, Color *inThumbColor,
                          Color *inBorderColor, double inBorderWidth, double inThumbWidth, double inMinThumbIncrement)
    : GUIComponentGL(inAnchorX, inAnchorY, inWidth, inHeight), mJustPressed(false), mJustReleased(false),
      mDragging(false), mIconWidthFraction(inIconWidthFraction), mBarStartColor(inBarStartColor),
      mBarEndColor(inBarEndColor), mThumbColor(inThumbColor), mBorderColor(inBorderColor), mThumbPosition(0.5),
      mBorderWidth(inBorderWidth), mThumbWidth(inThumbWidth), mMinThumbIncrement(inMinThumbIncrement)
{

    mThumbPosition = (int)(mThumbPosition / mMinThumbIncrement) * mMinThumbIncrement;
...
inline double SliderGL::getPeggedThumbPosition()
{
    ...
    return (int)(mThumbPosition * thumbUsableBarWidth / mMinThumbIncrement) * mMinThumbIncrement / thumbUsableBarWidth;
}
...
inline void SliderGL::mouseInput(double inX, double inY)
{
    if (mDragging)
    {
        ...
        mThumbPosition = (inX - (iconEndX + 0.5 * mThumbWidth)) / thumbUsableBarWidth;
...
inline void SliderGL::fireRedraw()
{
    ...
    glColor4f(mBorderColor->r, mBorderColor->g, mBorderColor->b, 1.0f);
    glBegin(GL_QUADS);
    {
    ...
    glBegin(GL_QUADS);
    {
        ...
        // start of bar
        glColor4f(mBarStartColor->r, mBarStartColor->g, mBarStartColor->b, 1.0f);
        glVertex2d(barStartX + mBorderWidth, barStartY + mBorderWidth);
        glVertex2d(barStartX + mBorderWidth, barStartY + barHeight - mBorderWidth);

        // end of bar
        glColor4f(mBarEndColor->r, mBarEndColor->g, mBarEndColor->b, 1.0f);
        glVertex2d(barStartX + barWidth - mBorderWidth, barStartY + barHeight - mBorderWidth);
        glVertex2d(barStartX + barWidth - mBorderWidth, barStartY + mBorderWidth);
    }
    glEnd();
    ...
    double thumbStartX = thumbDrawPosition * thumbUsableBarWidth + barStartX;
    ...
    // draw fill, smaller
    glColor4f(mThumbColor->r, mThumbColor->g, mThumbColor->b, 1.0f);
    glBegin(GL_QUADS);
    {
        glVertex2d(thumbStartX + mBorderWidth, mAnchorY + mBorderWidth);
        glVertex2d(thumbStartX + mBorderWidth, mAnchorY + mHeight - mBorderWidth);

        glVertex2d(thumbEndX - mBorderWidth, mAnchorY + mHeight - mBorderWidth);
