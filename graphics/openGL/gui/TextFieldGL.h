...
class TextFieldGL : public GUIComponentGL, public ActionListenerList
{
  ...
  public:
     ...
     *   Defaults to false.
     */
    TextFieldGL(double inAnchorX, double inAnchorY, double inWidth, double inHeight, double inBorderWidth,
                const char *inString, TextGL *inText, Color *inBorderColor, Color *inFocusedBorderColor,
                Color *inBackgroundColor, Color *inCursorColor, int inLengthLimit = -1, char inForceUppercase = false);
    ...
    virtual void keyReleased(unsigned char inKey, double inX, double inY);
...
inline TextFieldGL::TextFieldGL(double inAnchorX, double inAnchorY, double inWidth, double inHeight,
                                double inBorderWidth, const char *inString, TextGL *inText, Color *inBorderColor,
                                Color *inFocusedBorderColor, Color *inBackgroundColor, Color *inCursorColor,
                                int inLengthLimit, char inForceUppercase)
    : GUIComponentGL(inAnchorX, inAnchorY, inWidth, inHeight), mText(inText), mString(NULL),
      mBorderColor(inBorderColor), mFocusedBorderColor(inFocusedBorderColor), mBackgroundColor(inBackgroundColor),
      mCursorColor(inCursorColor), mCurrentBorderColor(inBorderColor), mLengthLimit(inLengthLimit),
      mForceUppercase(inForceUppercase), mFocused(false), mCursorPosition(0), mBorderWidth(inBorderWidth),
      mIgnoreNextKey(false), mPressStartedOnUs(false)
{
...
inline void TextFieldGL::keyPressed(unsigned char inKey, double inX, double inY)
{
    ...
    if (inKey == 127 || inKey == 8)
    {
        if (mCursorPosition != 0)
        {
            ...
            if (length != 0)
            {
                ...
                memcpy(&(newString[mCursorPosition - 1]), &(mString[mCursorPosition]), length - mCursorPosition + 1);
    ...
    else if (((inKey >= 32 && inKey <= 126) || (inKey >= 160)) &&
             (mLengthLimit < 0 || (int)strlen(mString) < mLengthLimit))
    {
        ...
        memcpy(&(newString[mCursorPosition + 1]), &(mString[mCursorPosition]), oldStringLength - mCursorPosition + 1);
...
inline void TextFieldGL::specialKeyPressed(int inKey, double inX, double inY)
{
...
inline void TextFieldGL::keyReleased(unsigned char inKey, double inX, double inY)
{
}

inline void TextFieldGL::specialKeyReleased(int inKey, double inX, double inY)
{
}
...
inline char TextFieldGL::isInside(double inX, double inY)
{
    ...
    if (inX >= mAnchorX - borderOffset && inX < mAnchorX + mWidth + borderOffset && inY >= mAnchorY - borderOffset &&
...
inline void TextFieldGL::mouseReleased(double inX, double inY)
{
    ...
    if (isInside(inX, inY) && mPressStartedOnUs)
    {
        ...
        for (int i = 0; i < numChars + 1 && !found; i++)
        {
            ...
            double subWidth = mText->measureTextWidth(substring) * mHeight + offset;
            ...
            if (i < numChars)
            {
                ...
                subWidth += mText->measureTextWidth(nextChar) * mHeight * 0.5;
            }
...
inline void TextFieldGL::fireRedraw()
{
    ...
    glColor4f(mCurrentBorderColor->r, mCurrentBorderColor->g, mCurrentBorderColor->b, 1.0f);
    glBegin(GL_QUADS);
    {
        glVertex2d(mAnchorX - borderOffset, mAnchorY - borderOffset);
        glVertex2d(mAnchorX - borderOffset, mAnchorY + mHeight + borderOffset);

        glVertex2d(mAnchorX + mWidth + borderOffset, mAnchorY + mHeight + borderOffset);
        glVertex2d(mAnchorX + mWidth + borderOffset, mAnchorY - borderOffset);
    }
    glEnd();

    // draw the background
    glColor4f(mBackgroundColor->r, mBackgroundColor->g, mBackgroundColor->b, 1.0f);
    glBegin(GL_QUADS);
    {
    ...
    // draw the text
    mText->drawText(mString, mAnchorX, mAnchorY, charWidth, mHeight);

    if (mFocused)
    {
        // draw the cursor
        glColor4f(mCursorColor->r, mCursorColor->g, mCursorColor->b, 1.0f);
        /*
        glBegin( GL_LINES ); {
