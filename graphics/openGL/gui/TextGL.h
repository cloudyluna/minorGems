...
class TextGL
{
  ...
  public:
     ...
     */
    TextGL(Image *inImage, char inUseAlpha = false, char inFixedWidth = true, double inCharacterSpacingFraction = 0.1,
     ...
     */
    void drawText(const char *inString, double inX, double inY, double inWidth, double inHeight);
  ...
  protected:
     ...
     */
    void drawCharacter(unsigned char inCharacter, double inX, double inY, double inWidth, double inHeight,
                       double *outUsedWidth);
     ...
     *   constructor.
     */
    void generateCharacterWidths(Image *inFontImage, char inFixedWidth, double inCharacterSpacingFraction,
                                 double inSpaceWidthFraction);
};

inline TextGL::TextGL(Image *inImage, char inUseAlpha, char inFixedWidth, double inCharacterSpacingFraction,
                      double inSpaceWidthFraction)

    : mXImageFractionUsed(1.0), mYImageFractionUsed(1.0), mFontColor(new Color(1.0f, 1.0f, 1.0f, 1.0f)),
      mGradientColor(NULL), mZ(0), mSmooth(false), mUseAlpha(inUseAlpha), mStartWidthFractionMetrics(new double[256]),
      mEndWidthFractionMetrics(new double[256]), mCharacterSpacingFraction(inCharacterSpacingFraction),
      mHeightFractionMetrics(new double[256])
{
    ...
    for (int c = 0; c < numChannels; c++)
    {
        ...
        {
            // copy row
            memcpy(&(destChannel[y * paddedW]), &(sourceChannel[y * w]), sizeof(double) * w);
        }
    }
    ...
    if (inUseAlpha && paddedImage.getNumChannels() != 4)
    {
        ...
        Image *fontImage = new Image(paddedImage.getWidth(), paddedImage.getHeight(), 4);

        int numPixels = paddedImage.getWidth() * paddedImage.getHeight();

        // copy the red channel of paddedImage into our alpha
        memcpy(fontImage->getChannel(3), paddedImage.getChannel(0), numPixels * sizeof(double));
        ...
        generateCharacterWidths(fontImage, inFixedWidth, inCharacterSpacingFraction, inSpaceWidthFraction);
    ...
    else
    {
        ...
        generateCharacterWidths(&paddedImage, inFixedWidth, inCharacterSpacingFraction, inSpaceWidthFraction);
    }
}
...
inline void TextGL::drawText(const char *inString, double inX, double inY, double inWidth, double inHeight)
{
    ...
    for (int i = 0; i < numChars; i++)
    {
        ...
        double usedWidth;
        drawCharacter(c, currentCharX, inY, charWidth, inHeight, &usedWidth);
...
inline void TextGL::drawCharacter(unsigned char inCharacter, double inX, double inY, double inWidth, double inHeight,
                                  double *outUsedWidth)
{
    *outUsedWidth = inWidth * (mEndWidthFractionMetrics[inCharacter] - mStartWidthFractionMetrics[inCharacter]);
    ...
    double textureStartX = mXImageFractionUsed * (inCharacter % 16) / 16.0;
    double textureStartY = mYImageFractionUsed * (inCharacter / 16) / 16.0 + mYImageFractionUsed / 16;
    ...
    glBegin(GL_QUADS);
    {
        if (mGradientColor != NULL)
        {
            glColor4f(mGradientColor->r, mGradientColor->g, mGradientColor->b, mGradientColor->a);
        }
        else
        {
            glColor4f(mFontColor->r, mFontColor->g, mFontColor->b, mFontColor->a);
        }
...
inline void TextGL::generateCharacterWidths(Image *inFontImage, char inFixedWidth, double inCharacterSpacingFraction,
                                            double inSpaceWidthFraction)
{
    if (mUseAlpha && !inFixedWidth)
    {
        ...
        int pixelsPerCharacterX = (int)(mXImageFractionUsed * imageWidth) / 16;

        int pixelsPerCharacterY = (int)(mYImageFractionUsed * imageHeight) / 16;
            ...
            for (int c = 0; c < 16; c++)
            {
                ...
                if (smallestXWithInk <= largestXWithInk)
                {
                    mStartWidthFractionMetrics[i] =
                        (double)(smallestXWithInk - charStartX) / (double)pixelsPerCharacterX;

                    mEndWidthFractionMetrics[i] = (double)(largestXWithInk - charStartX) / (double)pixelsPerCharacterX;

                    mStartWidthFractionMetrics[i] -= inCharacterSpacingFraction;

                    mEndWidthFractionMetrics[i] += inCharacterSpacingFraction;

                    // note:  base this on pixelsPerCharacterX,
                    // because height of 1.0 is basic block size, not
                    // extended (accent) size
                    mHeightFractionMetrics[i] = (double)(charEndY - smallestYWithInk) / (double)pixelsPerCharacterX;
                }
                else
                {
...
inline double TextGL::measureTextWidth(const char *inString)
{
    ...
    for (int i = 0; i < numChars; i++)
    {
        ...
        double charWidth = mEndWidthFractionMetrics[character] - mStartWidthFractionMetrics[character];
