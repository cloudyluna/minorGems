...
inline PipedStream::PipedStream()
    : mBuffers(new SimpleVector<unsigned char *>()), mBufferSizes(new SimpleVector<long>()), mCurrentBufferIndex(0)
{
}
...
inline long PipedStream::read(unsigned char *inBuffer, long inNumBytes)
{
    if (mBuffers->size() == 0)
    {
        ...
        InputStream::setNewLastErrorConst("No data available on piped stream read.");
        return 0;
    }
    else
    {
        ...
        while (outputIndex < inNumBytes)
        {
            ...
            if (bytesRemaining >= bytesRemainingInCurrent)
            {
                ...
                memcpy(&(inBuffer[outputIndex]), &(currentBuffer[mCurrentBufferIndex]), bytesRemainingInCurrent);
                ...
                if (outputIndex != inNumBytes && mBuffers->size() == 0)
                {
                    ...
                    InputStream::setNewLastErrorConst("Partial data available on piped stream read.");
                    return outputIndex;
                }
                if (mBuffers->size() != 0)
                {
            ...
            else
            {
                ...
                memcpy(&(inBuffer[outputIndex]), &(currentBuffer[mCurrentBufferIndex]), bytesRemaining);
