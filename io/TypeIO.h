...
class TypeIO
{
  ...
  public:
     ...
     * @param outBytes preallocated array where bytes will be returned.
     */
    static void doubleToBytes(double inDouble, unsigned char *outBytes);
...
inline void TypeIO::longToBytes(long inInt, unsigned char *outBytes)
{
...
{
    return (long)(inBytes[0] << 24 | inBytes[1] << 16 | inBytes[2] << 8 | inBytes[3]);
}

inline void TypeIO::shortToBytes(short inInt, unsigned char *outBytes)
{
    // use a big-endian conversion
