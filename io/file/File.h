...
class File
{
  ...
  public:
     ...
     *   Must be destroyed by caller.
     */
    unsigned char *readFileContents(int *outLength, char inTextMode = false);
...
inline File::File(Path *inPath, const char *inName, int inNameLength) : mPath(inPath), mNameLength(inNameLength)
{
            ...
            if (strcmp(entry->d_name, ".") && strcmp(entry->d_name, ".."))
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        int numSteps = 1;
                        char absolute = false;
                        newPath = new Path(folderPathArray, numSteps, absolute);
                ...
                fileVector->push_back(new File(newPath, entry->d_name, strlen(entry->d_name)));
            }
...
inline File **File::getChildFilesRecursive(int inDepthLimit, int *outNumFiles)
{
...
inline void File::getChildFilesRecursive(int inDepthLimit, SimpleVector<File *> *inResultVector)
{
        ...
        for (int i = 0; i < numChildren; i++)
        {
            ...
            if (child->isDirectory())
            {
                ...
                if (inDepthLimit > 0)
                {
                    ...
                    child->getChildFilesRecursive(inDepthLimit - 1, inResultVector);
                }
            }
        }
...
inline unsigned char *File::readFileContents(int *outLength, char inTextMode)
{
    ...
    if (exists())
    {
        ...
        if (returnData != NULL)
        {
            ...
            if (numRead == length || (inTextMode && numRead >= 0))
            {
                *outLength = numRead;
                return returnData;
            }
