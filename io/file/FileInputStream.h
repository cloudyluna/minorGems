...
inline FileInputStream::FileInputStream(File *inFile, char inTextMode) : mFile(inFile)
{
...
inline long FileInputStream::read(unsigned char *inBuffer, long inNumBytes)
{
    ...
    if (mUnderlyingFile != NULL)
    {
        ...
        if (numRead < inNumBytes)
        {
            ...
            if (feof(mUnderlyingFile))
            {
                ...
                char *stringBuffer = new char[fileNameLength + 50];
                sprintf(stringBuffer, "Reached end of file %s on read.", fileName);
                setNewLastError(stringBuffer);
            ...
            else
            {
                ...
                char *stringBuffer = new char[fileNameLength + 50];
                sprintf(stringBuffer, "Reading from file %s failed.", fileName);
                setNewLastError(stringBuffer);
    ...
    else
    {
        ...
        char *fileName = mFile->getFullFileName(&fileNameLength);
        char *stringBuffer = new char[fileNameLength + 50];
        sprintf(stringBuffer, "File %s was not opened properly before reading.", fileName);
        delete[] fileName;
