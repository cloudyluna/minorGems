...
inline Path::Path(char **inPathSteps, int inNumSteps, char inAbsolute, char *inRootString)
    : mNumSteps(inNumSteps), mAbsolute(inAbsolute), mRootString(NULL)
{
...
inline Path::Path(const char *inPathString)
{
    ...
    if (currentDelimPointer != NULL)
    {
        ...
        while (currentDelimPointer != NULL)
        {
            ...
            currentDelimPointer = strstr(&(currentDelimPointer[1]), delimString);
        }
        ...
        while (currentDelimPointer != NULL)
        {
            ...
            char *nextDelimPointer = strstr(&(currentDelimPointer[1]), delimString);
            ...
            mPathSteps[stepIndex] = stringDuplicate(&(currentDelimPointer[1]));
            mStepLength[stepIndex] = strlen(mPathSteps[stepIndex]);
...
inline Path *Path::append(const char *inStepString)
{
    ...
    Path *newPath = new Path(newPathSteps, mNumSteps + 1, mAbsolute, mRootString);
