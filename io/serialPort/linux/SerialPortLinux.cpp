...
SerialPort::SerialPort(int inBaud, int inParity, int inDataBits, int inStopBits)
{
    ...
    else
    {
        ...
        newTermIO.c_cflag = baudRate | parity | dataBits | stopBits | CLOCAL | CREAD;
...
SerialPort::~SerialPort()
{
    ...
    {
        LinuxSerialPortObject *serialPortObject = (LinuxSerialPortObject *)mNativeObjectPointer;
...
int SerialPort::sendLine(char *inLine)
{
    if (mNativeObjectPointer != NULL)
    {
        ...
        if (numWritten == stringLength && numWrittenEndLine == 1)
        {
            return 1;
        }
        else
