...
class ExpressionMutator
{
  ...
  public:
     ...
     */
    static Expression *mutateExpression(Expression *inExpression, double inMutationProb, double inMaxMutation,
                                        RandomSource *inRandSource);
};

inline Expression *ExpressionMutator::mutateExpression(Expression *inExpression, double inMutationProb,
                                                       double inMaxMutation, RandomSource *inRandSource)
{
    ...
    else if (expressionID == MultiConstantArgumentExpression::staticGetID())
    {
        ...
        Expression *wrappedExpression = m->extractWrappedExpression();

        wrappedExpression = mutateExpression(wrappedExpression, inMutationProb, inMaxMutation, inRandSource);
    ...
    else
    {
        ...
        if (inRandSource->getRandomDouble() <= inMutationProb)
        {
            ...
            long newID;
            if (expressionID == InvertExpression::staticGetID() || expressionID == NegateExpression::staticGetID() ||
        ...
        for (int i = 0; i < newExpression->getNumArguments(); i++)
        {
            ...
            // mutate the argument
            argument = mutateExpression(argument, inMutationProb, inMaxMutation, inRandSource);
