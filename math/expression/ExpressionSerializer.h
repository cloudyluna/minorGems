...
class ExpressionSerializer
{
  ...
  public:
     ...
     */
    static long serializeExpression(Expression *inExpression, OutputStream *inOutputStream);
     ...
     */
    static long deserializeExpression(Expression **outExpression, InputStream *inInputStream);
};

// these can both be implemented recursively... exciting!

inline long ExpressionSerializer::serializeExpression(Expression *inExpression, OutputStream *inOutputStream)
{
    ...
    {
        MultiConstantArgumentExpression *m = (MultiConstantArgumentExpression *)inExpression;

        Expression *wrappedExpression = m->getWrappedExpression();

        numBytes += serializeExpression(wrappedExpression, inOutputStream);
    }
    ...
    else
    {
        ...
        for (int i = 0; i < inExpression->getNumArguments(); i++)
        {
            ...
            numBytes += serializeExpression(inExpression->getArgument(i), inOutputStream);
        }
    }
...
inline long ExpressionSerializer::deserializeExpression(Expression **outExpression, InputStream *inInputStream)
{
    ...
    else if (expressionID == MultiConstantArgumentExpression::staticGetID())
    {
        ...
        numBytes += deserializeExpression(&wrappedExpression, inInputStream);

        MultiConstantArgumentExpression *m = new MultiConstantArgumentExpression(wrappedExpression);
    ...
    else
    {
        ...
        for (int i = 0; i < (*outExpression)->getNumArguments(); i++)
        {
            ...
            // call deserialize recursively
            Expression *argument;
            numBytes += deserializeExpression(&argument, inInputStream);
