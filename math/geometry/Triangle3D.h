...
class Triangle3D : public GeometricObject3D, public Serializable
{
  ...
  public:
     ...
     */
    Triangle3D(Vector3D *inPointA, Vector3D *inPointB, Vector3D *inPointC);
...
inline Triangle3D::Triangle3D(Vector3D *inEndpointA, Vector3D *inEndpointB, Vector3D *inEndpointC)
{
...
inline LineSegment3D *Triangle3D::getLineSegment(int inIndex)
{
    ...
    else
    {
        ...
        return new LineSegment3D(mPoints[inIndex], mPoints[(inIndex + 1) % 2]);
    }
}
...
inline Vector3D *Triangle3D::projectOntoPlane(Vector3D *inPoint)
{
    ...
    double minDistance = normal->mX * (inPoint->mX - mPoints[0]->mX) + normal->mY * (inPoint->mY - mPoints[0]->mY) +
                         normal->mZ * (inPoint->mZ - mPoints[0]->mZ);
    minDistance = minDistance / sqrt(normal->mX * normal->mX + normal->mY * normal->mY + normal->mZ * normal->mZ);
...
inline char Triangle3D::isInBounds(Vector3D *inPoint)
{
    ...
    angleSum += acos(firstLeg->dot(secondLeg) / (firstLeg->getLength() * secondLeg->getLength()));

    angleSum += acos(secondLeg->dot(thirdLeg) / (secondLeg->getLength() * thirdLeg->getLength()));

    angleSum += acos(thirdLeg->dot(firstLeg) / (thirdLeg->getLength() * firstLeg->getLength()));
