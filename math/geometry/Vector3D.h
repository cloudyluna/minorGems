...
class Vector3D : public Serializable
{
  ...
  public:
     ...
     */
    static Vector3D *linearSum(Vector3D *inFirst, Vector3D *inSecond, double inFirstWeight);
...
inline Vector3D::Vector3D(double inX, double inY, double inZ) : mX(inX), mY(inY), mZ(inZ)
{
}

inline Vector3D::Vector3D() : mX(0), mY(0), mZ(0)
{
}

inline Vector3D::Vector3D(Vector3D *inOther) : mX(inOther->mX), mY(inOther->mY), mZ(inOther->mZ)
{
}
...
{
    setCoordinates(inOther->mX, inOther->mY, inOther->mZ);
}
...
inline void Vector3D::rotate(Vector3D *inAxis, double inAngle)
{
    ...
    */
    ...
    double newX = (txx + c) * mX + (txy + sz) * mY + (txz - sy) * mZ;
    double newY = (txy - sz) * mX + (tyy + c) * mY + (tyz + sx) * mZ;
    double newZ = (txz + sy) * mX + (tyz - sx) * mY + (tzz + c) * mZ;
...
inline Vector3D *Vector3D::linearSum(Vector3D *inFirst, Vector3D *inSecond, double inFirstWeight)
{
...
{
    Angle3D *actualAngle = new Angle3D(-inAngle->mX, -inAngle->mY, -inAngle->mZ);
