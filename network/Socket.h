...
class Socket
{
  ...
  public:
     ...
     *   would block.
     */
    int send(unsigned char *inBuffer, int inNumBytes, char inAllowedToBlock = true, char inAllowDelay = true);
     ...
     */
    int receive(unsigned char *inBuffer, int inNumBytes, long inTimeout);
...
inline Socket::Socket() : mConnected(true), mIsConnectionBroken(false)
{
}
...
inline void Socket::sendFlushBeforeClose(int inMaxTimeInMilliseconds)
{
    ...
    {
        numRead = this->receive(tempBuffer, 1, timeout);

        // track total time whether the receive timed out or not
        totalTimeout = (long)(Time::getMillisecondsSince(startSec, startMsec));
    }
