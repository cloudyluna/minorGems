...
class SocketUDP
{
  ...
  public:
     ...
     */
    static struct UDPAddress *makeAddress(const char *inAddress, unsigned short inPort);
     ...
     */
    static char *extractAddress(struct UDPAddress *inAddress, unsigned short *outPort);
     ...
     *   different.
     */
    static char compare(struct UDPAddress *inFirst, struct UDPAddress *inSecond);
     ...
     */
    int send(struct UDPAddress *inAddress, unsigned char *inData, unsigned long inNumBytes);
     ...
     */
    int receive(struct UDPAddress **outAddress, unsigned char **outData, long inTimeout = -1);
...
inline char SocketUDP::compare(struct UDPAddress *inFirst, struct UDPAddress *inSecond)
{
    if (inFirst->mIPAddress == inSecond->mIPAddress && inFirst->mPort == inSecond->mPort)
    {
        return true;
    }
    else
