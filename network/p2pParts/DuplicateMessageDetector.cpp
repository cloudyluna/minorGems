...
DuplicateMessageDetector::DuplicateMessageDetector(int inMessageHistorySize)
    : mMaxHistorySize(inMessageHistorySize), mLock(new MutexLock()), mSeenIDs(new SimpleVector<char *>()),
      mTotalMessageCount(0), mHistoryOutputFile(NULL)
{
...
char DuplicateMessageDetector::checkIfMessageSeen(char *inMessageUniqueID)
{
    ...
    {
        fprintf(mHistoryOutputFile, "%d %d %s", mTotalMessageCount, (int)(time(NULL)), inMessageUniqueID);
    }
