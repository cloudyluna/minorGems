                        ...
                        void *inProgressHandlerExtraArgument, char *inDestinationPath)
{
    ...
    if (outputFile == NULL)
    {
        ...
        inDownloadProgressHandler(MULTISOURCE_DOWNLOAD_FAILED, 0, inProgressHandlerExtraArgument);
        return;
    }
    ...
    while (sourceIndex < inNumSources && !failed && !canceled && chunksSoFar < numChunks)
    {
        ...
        unsigned char *chunkData = inChunkGetter(source, inFileDescriptor, chunksSoFar, chunkSize);
        ...
        if (chunkData != NULL)
        {
            ...
            unsigned long numWritten = fwrite(chunkData, 1, chunkSize, outputFile);

            if (numWritten == chunkSize)
            {
                char shouldContinue = inDownloadProgressHandler(MULTISOURCE_DOWNLOAD_IN_PROGRESS, bytesSoFar,
                ...
                if (!shouldContinue)
                {
                    ...
                    inDownloadProgressHandler(MULTISOURCE_DOWNLOAD_CANCELED, bytesSoFar,
                                              inProgressHandlerExtraArgument);
                }
            }
            else
            {
                ...
                inDownloadProgressHandler(MULTISOURCE_DOWNLOAD_FAILED, bytesSoFar, inProgressHandlerExtraArgument);
                failed = true;
            }
            delete[] chunkData;
        }
    ...
    // failure
    if (!failed && !canceled && chunksSoFar < numChunks)
    {
