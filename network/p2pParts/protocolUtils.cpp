...
char *readStreamUpToTag(InputStream *inInputStream, char *inTag, int inMaxCharsToRead)
{
    ...
    {
        long numRead = inInputStream->readByte((unsigned char *)(&(readCharBuffer[numCharsRead])));
        ...
        else
        {
            ...
            {
                char *possibleBodyStart = &(readCharBuffer[numCharsRead - tagLength]);
    ...
    {
        char *message = autoSprintf("Failed to find end tag \"%s\", read %d characters:\n%s\n", inTag, numCharsRead,
                                    readCharBuffer);
...
char *readStreamUpToTagAndGetToken(InputStream *inInputStream, char *inTag, int inMaxCharsToRead, int inTokenNumber)
{
    ...
    // read the string
    char *readString = readStreamUpToTag(inInputStream, inTag, inMaxCharsToRead);
    ...
    SimpleVector<char *> *readTokens = tokenizeString(readString);
    ...
    {
        selectedToken = stringDuplicate(*(readTokens->getElement(inTokenNumber)));
    }
    else
    {
        char *message = autoSprintf("Looking for token %d, but only %d tokens available\n", inTokenNumber, numTokens);
