 ...
 */
int waitForIncomingData(int inSocketID, long inMilliseconds);
...
struct UDPAddress *SocketUDP::makeAddress(const char *inAddress, unsigned short inPort)
{
...
char *SocketUDP::extractAddress(struct UDPAddress *inAddress, unsigned short *outPort)
{
...
int SocketUDP::send(struct UDPAddress *inAddress, unsigned char *inData, unsigned long inNumBytes)
{
    ...
    return sendto(socketID, (char *)inData, inNumBytes, 0, (struct sockaddr *)(&toAddress), sizeof(toAddress));
}

int SocketUDP::receive(struct UDPAddress **outAddress, unsigned char **outData, long inTimeout)
{
    ...
    numReceived =
        recvfrom(socketID, (char *)receiveBuffer, bufferSize, 0, (struct sockaddr *)(&fromAddress), &fromAddressLength);
