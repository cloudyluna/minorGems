...
static int getIGDURL(int inTimeoutMS, struct UPNPUrls *urls, struct IGDdatas *data, char *lanIPAddr, int lanIPLength)
{
    ...
    int result = UPNP_GetValidIGD(devList, urls, data, lanIPAddr, lanIPLength);
...
int mapPort(int inPort, const char *inDescription, int inTimeoutMS, char **outExternalIP)
{
    ...
    *outExternalIP = NULL;
    ...
    int result = getIGDURL(inTimeoutMS, &urls, &data, lanIPAddr, 16);
    ...
    if (result > 0)
    {
        ...
        if (result == 1)
        {
            ...
            int result = UPNP_GetExternalIPAddress(urls.controlURL, data.servicetype, externalIPAddress);

            if (result == UPNPCOMMAND_SUCCESS && externalIPAddress[0] != '\0')
            {
            ...
            result = UPNP_AddPortMapping(urls.controlURL, data.servicetype, port, port, lanIPAddr, inDescription, "TCP",
                                         NULL);
            if (result != UPNPCOMMAND_SUCCESS)
            {

                AppLog::getLog()->logPrintf(Log::ERROR_LEVEL,
...
int unmapPort(int inPort, int inTimeoutMS)
{
    ...
    if (result > 0)
    {
        ...
        if (result == 1)
        {
            ...
            int result = UPNP_DeletePortMapping(urls.controlURL, data.servicetype, port, "TCP", NULL);
