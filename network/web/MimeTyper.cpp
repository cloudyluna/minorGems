...
MimeTyper::MimeTyper(char *inConfigFileName)
{
    ...
    {
        configFile = new File(new Path(pathSteps, 1, false), inConfigFileName);
    }
    ...
    else
    {
        ...
        int numRead = inStream->read((unsigned char *)mMimeTypesString, length);
...
char *MimeTyper::getMimeType(char *inFileExtension)
{
    ...
    if (extensionMatch)
    {
        ...
        if (numRead == 1)
        {
            ...
            numRead = sscanf(&(extensionMatch[numToSkip]), "%s", buffer);
            if (numRead == 1)
            {
