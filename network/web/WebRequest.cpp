...
WebRequest::WebRequest(const char *inMethod, const char *inURL, const char *inBody, const char *inProxy)
    : mError(false), mURL(stringDuplicate(inURL)), mRequest(NULL), mRequestPosition(-1), mResultReady(false),
      mResult(NULL), mSock(NULL)
{
    ...
    mSuppliedAddress = new HostAddress(stringDuplicate(serverNameCopy), portNumber);
    ...
    if (inBody != NULL)
    {
        ...
        delete[] lengthString;
        tempStream.writeString("Content-Type: application/x-www-form-urlencoded\r\n\r\n");
...
int WebRequest::step()
{
    ...
    if (mSock == NULL)
    {
        ...
        if (mLookupThread->isLookupDone())
        {
            ...
            if (mNumericalAddress != NULL)
            {
                ...
                mSock = SocketClient::connectToServer(mNumericalAddress, 0, &timedOut);
            ...
            if (mError)
            {
                ...
                else
                {
                    printf("Error:  "
                           ...
                           "socket to %s:%d\n",
                           mNumericalAddress->mAddressString, mNumericalAddress->mPort);
                }
    ...
    else if (connectStatus == 1)
    {
        ...
        if (mRequestPosition < (int)(strlen(mRequest)))
        {
            ...
            int numSent = mSock->send((unsigned char *)remainingRequest, strlen(remainingRequest),
                                      // non-blocking
                                      false);
            if (numSent == -1)
            {
        ...
        else
        {
            ...
            if (numRead == -1)
            {
                ...
                if (stringLocateIgnoreCase(responseString, "404 Not Found") != NULL)
                {
                ...
                char *contentStart = strstr(responseString, contentStartString);

                if (contentStart != NULL)
                {
                    // skip start string
                    char *content = &(contentStart[strlen(contentStartString)]);

                    // pointer arithmetic:
                    // ugly, but we don't know that result does not contain
                    // \0, so we can't use streln to find result length
                    int resultLength =
                        responseLength - strlen(contentStartString) - (int)(contentStart - responseString);

                    mResultSize = resultLength;

                    mResult = new char[resultLength + 1];
                    memcpy(mResult, content, resultLength);
