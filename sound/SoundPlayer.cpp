...
static int portaudioCallback(void *inputBuffer, void *outputBuffer, unsigned long framesPerBuffer, PaTimestamp outTime,
                             void *userData)
{
...
SoundPlayer::SoundPlayer(int inSampleRate, char inStereo, SampleSource *inSampleSource)
    : mSampleRate(inSampleRate), mStereo(inStereo), mSampleSource(inSampleSource)
{
    ...
    if (error == paNoError)
    {
        ...
        error = Pa_OpenStream(&mAudioStream,
