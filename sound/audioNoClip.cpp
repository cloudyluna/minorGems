...
NoClip resetAudioNoClip(double inMaxVolume, int inHoldTimeInSamples, int inDecayTimeInSamples)
{
...
void audioNoClip(NoClip *inC, double *inSamplesL, double *inSamplesR, int inNumSamples)
{
    ...
    for (int i = 0; i < inNumSamples; i++)
    {
        ...
        if (inC->gain != 1.0 || maxVal > inC->maxVolume)
        {
            ...
            if (inC->gain != 1.0 && (inC->gain + inC->gainDecayPerSample) * maxVal <= inC->maxVolume)
            {
