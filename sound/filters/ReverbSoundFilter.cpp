...
ReverbSoundFilter::ReverbSoundFilter(unsigned long inDelayInSamples, double inGain, int inSampleRate)
    : mDelayBuffer(new SoundSamples(inDelayInSamples)), mGain(inGain), mDelayBufferPosition(0),
      mLowPassStateL(initLowPass(500, inSampleRate, 0.7)), mLowPassStateR(initLowPass(500, inSampleRate, 0.7))
{
}
...
SoundSamples *ReverbSoundFilter::filterSamples(SoundSamples *inSamples)
{
    ...
    for (unsigned long i = 0; i < numSamples; i++)
    {
        ...
        outputSamples->mLeftChannel[i] += mDelayBuffer->mLeftChannel[mDelayBufferPosition];
        outputSamples->mRightChannel[i] += mDelayBuffer->mRightChannel[mDelayBufferPosition];

        // save our gained output in the delay buffer
        mDelayBuffer->mLeftChannel[mDelayBufferPosition] =
            mGain * coeffFilter(outputSamples->mLeftChannel[i], &mLowPassStateL);
        mDelayBuffer->mRightChannel[mDelayBufferPosition] =
            mGain * coeffFilter(outputSamples->mRightChannel[i], &mLowPassStateR);
