                             ...
                             int *outHeaderLength)
{
    ...
    */
    ...
    int soundSizeInBytes = (inNumChannels * inNumSampleFrames * inSampleSizeInBits) / 8;
    ...
    headerStream->writeLong(inSampleRateInHertz | 16398 << 16); // sample rate in Hz plus
...
int16_t *readMono16AIFFData(unsigned char *inData, int inNumBytes, int *outNumSamples, int *outSampleRate)
{
    ...
    int numSamples = inData[22] << 24 | inData[23] << 16 | inData[24] << 8 | inData[25];

    int sampleRate = inData[30] << 8 | inData[31];
    ...
    {
        samples[i] = (inData[b] << 8) | inData[b + 1];
        b += 2;
    }
