...
int main()
{
    ...
    // generate the header
    int headerSize;
    unsigned char *aiffHeader = getAIFFHeader(2, sampleSize, sampleRate, numNotes * samplesPerNote, &headerSize);
    ...
    for (int i = 0; i < numNotes; i++)
    {
        ...
        for (int j = 0; j < samplesPerNote; j++)
        {
            ...
            double sinVal = sin((((float)j) / (float)sampleRate) * noteFrequency * 2 * M_PI);
            /*
            sinVal = sinVal/3 +
