...
FinishedSignalThreadManager::FinishedSignalThreadManager()
    : mLock(new MutexLock()), mThreadVector(new SimpleVector<FinishedSignalThread *>()), mStopSignal(false),
...
void FinishedSignalThreadManager::addThread(FinishedSignalThread *inThread)
{
    ...
    while (!stopped)
    {
        ...
        while (foundFinished)
        {
            ...
            for (int i = 0; i < numThreads && !foundFinished; i++)
            {
                ...
                FinishedSignalThread *currentThread = *(mThreadVector->getElement(i));
