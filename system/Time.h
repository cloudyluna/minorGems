...
class Time
{
  public:
     ...
     */
    static void getCurrentTime(timeSec_t *outSeconds, unsigned long *outMilliseconds);
     ...
     */
    static unsigned long getMillisecondsSince(timeSec_t inSeconds, unsigned long inMilliseconds);
...
inline char Time::stringToTimeStruct(const char *inString, struct tm *inStruct)
{
    // strptime not available everywhere

    int year, month, day, hours, minutes, seconds;

    int numScanned = sscanf(inString, "%d-%d-%dT%d:%d:%dZ", &year, &month, &day, &hours, &minutes, &seconds);
...
inline void Time::setEpochTime()
{
    ...
    const char *epochStrings[4] = {"1970-01-01T00:00:00Z", "1980-01-01T00:00:00Z", "1990-01-01T00:00:00Z",
    ...
    for (int e = 0; e < 4; e++)
    {
        ...
        char gotStruct = stringToTimeStruct(epochStrings[e], &localTimeStruct);
        ...
        if (gotStruct)
        {
            ...
            if (localEpochTime != (time_t)(-1))
            {
                ...
                // to get gmt time
                int diff =
                    ((localTimeStruct.tm_hour - gmtStruct.tm_hour) * 60 + (localTimeStruct.tm_min - gmtStruct.tm_min)) *
                        60 +
...
inline unsigned long Time::getMillisecondsSince(timeSec_t inSeconds, unsigned long inMilliseconds)
{
