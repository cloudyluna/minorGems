...
template <class Type> class MinPriorityQueue
{
  public:
    int size()
    {
    ...
    void printHeap(int inNextParentIndex = 0, int depth = 0, char inSecondChild = false, char *inSkipVertLineMap = NULL)
    {
...
#define MAX_PRINT_HEAP_DEPTH 9999
        ...
        {
            memcpy(skipVertLineMap, inSkipVertLineMap, MAX_PRINT_HEAP_DEPTH);
        }
        else
        {
