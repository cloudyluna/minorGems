...
template <class Type> class SimpleVector
{
  public:
     ...
     *   Defaults to "unnamed".
     */
    void setPrintMessageOnVectorExpansion(char inPrintMessage, const char *inVectorName = "unnamed");
...
template <class Type> inline SimpleVector<Type>::SimpleVector(int sizeEstimate) : vectorName("")
{
...
inline SimpleVector<Type>::SimpleVector(const SimpleVector<Type> &inCopy)
    : elements(new Type[inCopy.maxSize]), numFilledElements(inCopy.numFilledElements), maxSize(inCopy.maxSize),
      minSize(inCopy.minSize), printExpansionMessage(inCopy.printExpansionMessage), vectorName(inCopy.vectorName)
{
...
// assignment operator
template <class Type> inline SimpleVector<Type> &SimpleVector<Type>::operator=(const SimpleVector<Type> &inOther)
{
...
template <class Type> inline Type SimpleVector<Type>::getElementDirect(int index)
{
    if (index < numFilledElements && index >= 0)
    {
        return elements[index];
...
template <class Type> inline Type SimpleVector<Type>::getElementDirectFast(int index)
{
    return elements[index];
}
...
{
    if (index < numFilledElements)
    { // if index valid for this vector
    ...
    { // index not valid for this vector
        return false;
    }
}

template <class Type> inline bool SimpleVector<Type>::deleteStartElements(int inNumToDelete)
{
    ...
    { // not enough eleements in vector
        return false;
    }
}
...
template <> inline bool SimpleVector<unsigned char>::deleteStartElements(int inNumToDelete)
{
    ...
    if (inNumToDelete <= numFilledElements)
    {
        ...
        if (inNumToDelete != numFilledElements)
        {
            ...
            memmove(elements, &(elements[inNumToDelete]), numFilledElements - inNumToDelete);
        }
    ...
    { // not enough elements in vector
        return false;
    }
}

// same for signed char vectors
template <> inline bool SimpleVector<char>::deleteStartElements(int inNumToDelete)
{
...
template <class Type> inline bool SimpleVector<Type>::deleteElementEqualTo(Type inElement)
{
...
template <class Type> inline void SimpleVector<Type>::swap(int inA, int inB)
{
    ...
    if (inA < numFilledElements && inA >= 0 && inB < numFilledElements && inB >= 0)
    {
...
template <class Type> inline int SimpleVector<Type>::getElementIndex(Type inElement)
{
...
template <class Type> inline void SimpleVector<Type>::deleteAll()
{
    numFilledElements = 0;
    if (maxSize > minSize)
    { // free memory if vector has grown
        delete[] elements;
...
{
    if (numFilledElements < maxSize)
    { // still room in vector
...
template <class Type> inline void SimpleVector<Type>::push_middle(Type x, int inNumBefore)
{
...
template <class Type> inline void SimpleVector<Type>::push_back(Type *inArray, int inLength)
{
...
template <class Type> inline void SimpleVector<Type>::push_back_other(SimpleVector<Type> *inOtherVector)
{
...
template <> inline void SimpleVector<char>::appendElementString(const char *inString)
{
    unsigned int numChars = strlen(inString);
    appendArray((char *)inString, (int)numChars);
}

template <> inline void SimpleVector<char>::setElementString(const char *inString)
{
    deleteAll();
...
template <class Type> inline void SimpleVector<Type>::appendArray(Type *inArray, int inSize)
{
    // slow but correct
...
template <class Type>
inline void SimpleVector<Type>::setPrintMessageOnVectorExpansion(char inPrintMessage, const char *inVectorName)
{
...
template <> inline char SimpleVector<char *>::deallocateStringElement(int inIndex)
{
