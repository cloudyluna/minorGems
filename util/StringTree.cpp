...
class StringTreeNode
{
  public:
    ...
    void insert(const char *inString, void *inValue, ValueHashTable *inHashTable);
    ...
    // returns num gotten
    void getValuesBelow(int *inNumToGet, int *inNumToSkip, SimpleVector<void *> *outValues, char inDownOnly);
...
StringTreeNode::StringTreeNode(char inChar, StringTreeNode *inParent)
    : mChar(inChar), mParent(inParent), mLeft(NULL), mDown(NULL), mRight(NULL)
{
}
...
void StringTreeNode::insert(const char *inString, void *inValue, ValueHashTable *inHashTable)
{
...
{
    if (mValues.size() == 0 && mLeft == NULL && mDown == NULL && mRight == NULL)
    {
...
void StringTreeNode::getValuesBelow(int *inNumToGet, int *inNumToSkip, SimpleVector<void *> *outValues, char inDownOnly)
{

    if (!inDownOnly && mLeft != NULL)
    {
        mLeft->getValuesBelow(inNumToGet, inNumToSkip, outValues, false);
    }
    ...
    if (mDown != NULL)
    {
        mDown->getValuesBelow(inNumToGet, inNumToSkip, outValues, false);
    }
    ...
    if (!inDownOnly && mRight != NULL)
    {
        mRight->getValuesBelow(inNumToGet, inNumToSkip, outValues, false);
    }
}
...
void StringTreeNode::print()
{
    ...
    */
    ...
    printf("n%p; n%p [label = \"%c (%s) {%p}\"]; ", (void *)this, (void *)this, mChar, valueString, (void *)this);
    ...
    {
        mLeft->print();
        printf("n%p -> n%p [label = \"L\", weight=1]; ", (void *)this, (void *)mLeft);
    }
    if (mDown != NULL)
    {
        mDown->print();
        printf("n%p -> n%p [label = \"D\", weight=4]; ", (void *)this, (void *)mDown);
    }
    if (mRight != NULL)
    {
        mRight->print();
        printf("n%p -> n%p [label = \"R\", weight=1]; ", (void *)this, (void *)mRight);
    }
}

StringTree::StringTree() : mTreeRoot(NULL), mHashTable(new ValueHashTable)
{
}
...
int StringTree::getMatches(const char *inSearch, int inNumToSkip, int inNumToGet, void **outValues)
{
    ...
    matchNode->getValuesBelow(&numToGet, &numToSkip, &results, downOnly);
