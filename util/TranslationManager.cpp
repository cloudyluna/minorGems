...
{
    mStaticMembers.setDirectoryAndLanguage(inName, mStaticMembers.mLanguageName, true);
}
...
void TranslationManager::setLanguage(const char *inLanguageName, char inClearOldKeys)
{
    mStaticMembers.setDirectoryAndLanguage(mStaticMembers.mDirectoryName, inLanguageName, inClearOldKeys);
}

void TranslationManager::setLanguageData(const char *inData, char inClearOldKeys)
{
...
const char *TranslationManager::translate(const char *inTranslationKey)
{
    ...
    SimpleVector<char *> *keys = mStaticMembers.mTranslationKeys;

    SimpleVector<char *> *naturalLanguageStrings = mStaticMembers.mNaturalLanguageStrings;
    ...
    if (keys != NULL)
    {
        ...
        for (int i = 0; i < numKeys && translatedString == NULL; i++)
        {
            ...
            {
                // keys match
                translatedString = *(naturalLanguageStrings->getElement(i));
            }
        }
    }
...
TranslationManagerStaticMembers::TranslationManagerStaticMembers()
    : mDirectoryName(NULL), mLanguageName(NULL), mTranslationKeys(NULL), mNaturalLanguageStrings(NULL)
{
...
void TranslationManagerStaticMembers::setDirectoryAndLanguage(const char *inDirectoryName, const char *inLanguageName,
                                                              char inClearOldKeys)
{
...
void TranslationManagerStaticMembers::setTranslationData(const char *inData, char inClearOldKeys)
{
    ...
    while (!readError)
    {
        ...
        if (numRead == 1)
        {
            ...
            if (readChar != EOF)
            {
                ...
                numRead = sscanf(inData, "%999[^\"]", naturalLanguageString);

                if (numRead == 1)
                {
                    inData = stringSkip(inData, strlen(naturalLanguageString));
                    ...
                    {
                        if (strcmp(*(mTranslationKeys->getElement(i)), trimmedKey) == 0)
                        {
                            keyExists = true;
                            break;
                        }
                    ...
                    if (!keyExists)
                    {
                        ...
                        mNaturalLanguageStrings->push_back(stringDuplicate(naturalLanguageString));
                    }
                    else
                    {
                        delete[] trimmedKey;
