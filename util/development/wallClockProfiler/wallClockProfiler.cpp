...
static int fillBufferWithResponse()
{
    ...
    while (true)
    {
        int numRead = read(inPipe, &(readBuff[readSoFar]), 4095 - readSoFar);
...
static char stackCompare(Stack *inA, Stack *inB)
{
    ...
    {
        if (inA->frames.getElementDirect(i).address != inB->frames.getElementDirect(i).address)
        {
            return false;
        }
    }
...
int main(int inNumArgs, char **inArgs)
{
    ...
    else if (childPID == 0)
    {
        ...
        {
            // attatch to PID
            execlp("gdb", "gdb", "-nx", "--interpreter=mi", inArgs[2], inArgs[3], NULL);
        }
        else
        {
    ...
    while (!programExited && (detatchSeconds == -1 || time(NULL) < startTime + detatchSeconds))
    {
        usleep(usPerSample);
    ...
    for (int i = 0; i < sortedStacks.size(); i++)
    {
        ...
        printf("%6.3f%% =====================================\n"
               "      %3d: %s   (at %s:%d)\n",
               100 * s.sampleCount / (float)numSamples, 1, s.frames.getElement(0)->funcName,
               s.frames.getElement(0)->fileName, s.frames.getElement(0)->lineNum);
        // print stack for context below
        for (int j = 1; j < s.frames.size(); j++)
        {
            StackFrame f = s.frames.getElementDirect(j);
            printf("      %3d: %s   (at %s:%d)\n", j + 1, f.funcName, f.fileName, f.lineNum);
        }
        printf("\n\n");
