...
class Log
{
  ...
  public:
     ...
     */
    virtual void logString(int inLevel, const char *inFormatString, ...) = 0;

    // for backwards compatibility
    virtual void logPrintf(int inLevel, const char *inFormatString, ...) = 0;
     ...
     *   Must be destroyed by caller.
     */
    virtual void logString(const char *inLoggerName, int inLevel, const char *inFormatString, ...) = 0;

    // versions that take var-arg-lists explicitly
    // lists must be va_end-ed by caller.

    virtual void logStringV(int inLevel, const char *inFormatString, va_list inArgList) = 0;

    virtual void logStringV(const char *inLoggerName, int inLevel, const char *inFormatString, va_list inArgList) = 0;
