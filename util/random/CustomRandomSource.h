...
inline CustomRandomSource::CustomRandomSource() : mState((unsigned)(time(NULL)))
{

    saveState();
}

inline CustomRandomSource::CustomRandomSource(unsigned int inSeed) : mState(inSeed)
{
    saveState();
}
...
inline void CustomRandomSource::restoreFromSavedState(unsigned int inSavedState)
{
...
#define CustNum1(inSeed) ((inSeed * 0xFEA09B9DU) + 1)

#define CustNum2(inSeed) (((inSeed ^ CustNum1(inSeed)) * 0x9C129511U) + 1)

#define CustNum3(inSeed) ((inSeed * 0x2512CFB8U) + 1)

#define CustNum4(inSeed) (((inSeed ^ CustNum3(inSeed)) * 0xB89C8895U) + 1)

#define CustNum5(inSeed) ((inSeed * 0x6BF962C1U) + 1)

#define CustNum6(inSeed) (((inSeed ^ CustNum5(inSeed)) * 0x4BF962C1U) + 1)

inline unsigned int CustomRandomSource::genRand32()
{
    mState = CustNum2(mState) ^ (CustNum4(mState) >> 11) ^ (CustNum6(mState) >> 22);
