            ...
            while (buffX < buffWide)
            {
                ...
                while (blockY < blockHigh)
                {
                    ...
                    while (blockX < blockWide)
                    {
                        ...
                        blockX++;
                        if (buffX >= buffWide)
                            blockX = blockWide; // if this block hangs outside buffer
                    }
                    buffY++;
                    blockY++;
                    if (buffY >= buffHigh)
                        blockY = blockHigh; // if this block hangs outside buffer
                }
...
void genFractalNoise2d(double *inBuffer, int inWidth, int inMaxFrequency, double inFPower, char inInterpolate,
    ...
    for (f = 2; f <= inMaxFrequency; f = f * 2)
    {
        ...
        for (y = 0; y < w; y++)
        {
            ...
            for (x = 0; x < w; x++)
            {
                ...
                if (inInterpolate)
                {
                    value = xWeight * (yWeight * blockValues[yBlock * f + xBlock] +
                                       (1 - yWeight) * blockValues[(yBlock - 1) * f + xBlock]) +
                            (1 - xWeight) * (yWeight * blockValues[yBlock * f + xBlock - 1] +
                                             (1 - yWeight) * blockValues[(yBlock - 1) * f + xBlock - 1]);
                }
                else
                {
                    value = blockValues[yBlock * f + xBlock];
                }
...
void genFractalNoise(double *inBuffer, int inWidth, int inMaxFrequency, double inFPower, char inInterpolate,
                     RandomSource *inRandSource)
{
    ...
    for (f = 2; f <= inMaxFrequency; f = f * 2)
    {
        ...
        for (x = 0; x < w; x++)
        {
            ...
            if (inInterpolate)
            {
                value = xWeight * blockValues[xBlock] + (1 - xWeight) * blockValues[xBlock - 1];
            }
            else
            {
                value = blockValues[xBlock];
            }
