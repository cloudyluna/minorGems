...
#define maxRunLengthToTrack 100
    ...
    for (i = 0; i < numSteps; i++)
    {
        ...
        for (int b = 0; b < bitsPerStep; b++)
        {
            ...
            else
            {
                ...
                if (currentRunLength > 0 && currentRunLength < maxRunLengthToTrack)
                {
                    // count it
    ...
    for (j = 1; j < numCValues; j++)
    {
        ...
        c[j] = 8 * drawnBits[4 * j - 3] + 4 * drawnBits[4 * j - 2] + 2 * drawnBits[4 * j - 1] + 1 * drawnBits[4 * j];
...
#define numBins 7
    ...
    for (int i = 0; i < numBins; i++)
    {
        printf("Bin %d    Count = %d   Percent=%0.4f%%\n", i, bins[i], 100.0 * bins[i] / (float)totalCount);
    }
}
