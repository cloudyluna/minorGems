...
char *stringLocateIgnoreCase(const char *inHaystack, const char *inNeedle)
{
    ...
    if (matchPointer != NULL)
    {
        ...
        unsigned int haystackIndex = strlen(inHaystack) - matchRemainderLength;
...
char *replaceOnce(const char *inHaystack, const char *inTarget, const char *inSubstitute, char *outFound,
                  int inSkipChars, int *outAfterReplacementIndex)
{

    char *haystackCopy = stringDuplicate(inHaystack);

    char *fieldTargetPointer = strstr(&(haystackCopy[inSkipChars]), inTarget);
    ...
    else
    {
        ...
        char *fieldPostTargetPointer = &(fieldTargetPointer[strlen(inTarget)]);

        char *returnString = new char[strlen(inHaystack) - strlen(inTarget) + strlen(inSubstitute) + 1];

        sprintf(returnString, "%s%s%s", haystackCopy, inSubstitute, fieldPostTargetPointer);

        if (outAfterReplacementIndex != NULL)
        {
            *outAfterReplacementIndex = strlen(haystackCopy) + strlen(inSubstitute);
        }
...
char *replaceAll(const char *inHaystack, const char *inTarget, const char *inSubstitute, char *outFound)
{
    ...
    while (lastFound)
    {
        ...
        char *nextReturnString = replaceOnce(returnString, inTarget, inSubstitute, &lastFound, skip, &nextSkip);
                                          ...
                                          SimpleVector<char *> *inSubstituteVector)
{
    ...
    for (int i = 0; i < numTargets; i++)
    {
        char *newHaystackWithReplacements =
            replaceAll(newHaystack, *(inTargetVector->getElement(i)), *(inSubstituteVector->getElement(i)), &tagFound);
        delete[] newHaystack;
...
SimpleVector<char *> *tokenizeString(const char *inString)
{
    ...
    SimpleVector<char *> *foundTokens = new SimpleVector<char *>(numTokensGuess);
...
char *trimWhitespace(char *inString)
{
    ...
    while (length != 0 && (tempString[length - 1] == ' ' || tempString[length - 1] == '\n' ||
                           tempString[length - 1] == '\r' || tempString[length - 1] == '\t'))
    {
...
char *vautoSprintf(const char *inFormatString, va_list inArgList)
{
    ...
    else
    {
        ...
        while (stringLength == -1 || (unsigned int)stringLength == bufferSize)
        {
            ...
            stringLength = vsnprintf(buffer, bufferSize, inFormatString, argListCopyB);
...
int scanIntAndSkip(char **inOutStringPointer, char *outSuccess)
{
